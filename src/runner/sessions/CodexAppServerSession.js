...
import { detectCodexAuthIssue } from '../../lib/codexAuth.js'
...
export class CodexAppServerSession {
   ...
   *   onAuthIssue?: (issue: any) => void,
   * }} opts
   */
  constructor({ sessionId, cwd, options, debug = null, emit, onAuthIssue = null }) {
    this.sessionId = sessionId
    this.cwd = cwd
    this.options = options ?? null
    this.emit = emit
    this.onAuthIssue = typeof onAuthIssue === 'function' ? onAuthIssue : null
    ...
    this.stderrAuthBuffer = ''
    this.lastReportedAuthIssueCode = null
    ...
    this.rpc = new JsonRpcStdioClient({
      ...
      onStderr: (chunk) => {
        this.stderrAuthBuffer = `${this.stderrAuthBuffer}${String(chunk ?? '')}`.slice(-8_192)
        this.#reportAuthIssue(this.stderrAuthBuffer)
  ...
  #reportAuthIssue(value) {
    const issue = detectCodexAuthIssue(value)
    if (!issue) return

    const issueCode = safeString(issue.code) ?? safeString(issue.message)
    if (issueCode && issueCode === this.lastReportedAuthIssueCode) return
    this.lastReportedAuthIssueCode = issueCode ?? this.lastReportedAuthIssueCode

    try {
      this.onAuthIssue?.(issue)
    } catch {
    }
    ...
    this.emit('session.error', {
      ...
      authStatus: issue.status ?? null,
      message: issue.message,
      details: 'Use Settings > Machines > Runner tools > Codex to start ChatGPT sign-in again, or run codex logout followed by codex login on the runner shell.',
      willRetry: false
    })
  }
  ...
  async start(input, { threadId = null } = {}) {
    ...
    const serviceTiers = serviceTierCandidates(this.options?.serviceTier)
   ...
   * @param {{ model?: string|null, reasoningEffort?: string|null, serviceTier?: string|null, approvalPolicy?: string|null, sandbox?: string|null }} patch
   */
  updateOptions(patch) {
    ...
    if ('serviceTier' in patch) next.serviceTier = (typeof patch.serviceTier === 'string' && patch.serviceTier.trim()) ? patch.serviceTier.trim() : (patch.serviceTier ?? null)
  ...
  #onNotification(msg) {
    ...
    if (method === 'error') {
      ...
      this.#reportAuthIssue([message, details, params?.error?.message].filter(Boolean).join('\n'))
    ...
    if (method === 'account/rateLimits/updated') {
      const event = buildRateLimitsUpdatedEvent({
        sessionId: this.sessionId,
        params
      })
      if (!event) return
      this.emit('account.rateLimits.updated', event)
      return
    }
