...
export function serviceTierCandidates(input) {
  if (!input) return []
  const raw = String(input).trim()
  if (!raw) return []
...
function pushServiceTierVariants(out, base, serviceTiers, reasoningEfforts) {
  const tiers = Array.isArray(serviceTiers) ? serviceTiers : []
  if (tiers.length) {
    for (const serviceTier of tiers) {
      pushParamVariants(out, { ...base, serviceTier }, reasoningEfforts)
      pushParamVariants(out, { ...base, service_tier: serviceTier }, reasoningEfforts)
    }
  }
  pushParamVariants(out, base, reasoningEfforts)
}
...
export function buildResumeThreadAttempts({
  ...
  for (const approvalPolicy of approvalVals) {
    for (const sandbox of sandboxVals) {
      ...
      pushServiceTierVariants(out, { threadId, ...common }, serviceTiers, reasoningEfforts)
      pushServiceTierVariants(out, { id: threadId, ...common }, serviceTiers, reasoningEfforts)
    }
  }
