...
import { getManagedRunnerToolBinPath } from '../lib/runnerTooling.js'
...
function appendOutputHead(current, chunk, maxChars = 400_000) {
...
async function runCommandCaptureHead(command, args, {
  cwd = homedir(),
  timeoutMs = 20_000,
  env = process.env,
  maxChars = 400_000
  ...
  return await new Promise((resolve, reject) => {
    ...
    let timedOut = false
    let truncated = false
    const cap = Math.max(1_000, Math.min(Number(maxChars) || 400_000, 2_000_000))
    const timer = timeoutMs > 0
      ? setTimeout(() => {
        timedOut = true
        try { proc.kill('SIGKILL') } catch { }
    ...
    proc.stdout?.on('data', (chunk) => {
      const next = appendOutputHead(stdout, chunk, cap)
      truncated ||= next.length < (stdout.length + String(chunk ?? '').length)
      stdout = next
    })
    proc.stderr?.on('data', (chunk) => {
      const next = appendOutputHead(stderr, chunk, cap)
      truncated ||= next.length < (stderr.length + String(chunk ?? '').length)
      stderr = next
    })
    proc.once('error', (err) => {
      if (timer) clearTimeout(timer)
...
async function runGitCommand(args, { cwd = '', timeoutMs = 10_000, env = process.env } = {}) {
...
async function getGitDiff({
  ...
  const result = await runCommandCaptureHead('git', args, {
    cwd: resolvedCwd,
    timeoutMs,
    maxChars
  })
  if (result.code !== 0) {
    throw new Error(String(result.stderr ?? result.stdout ?? 'git diff failed').trim() || 'git diff failed')
  }
  return {
    cwd: resolvedCwd,
    diff: result.stdout,
...
export function parseGitRemotesOutput(stdout) {
  return String(stdout ?? '')
    .split(/\r?\n/)
...
export async function listGitRemotes({ cwd = '', timeoutMs = 10_000 } = {}) {
  const result = await runGitCommand(['remote'], { cwd, timeoutMs })
  return {
    cwd: result.cwd,
    remotes: parseGitRemotesOutput(result.stdout)
  }
}
...
export async function getGitStatus({ cwd = '', timeoutMs = 10_000 } = {}) {
  ...
  let remotes = []
  try {
    const [branchInfo, remoteInfo] = await Promise.all([
      listGitBranches({ cwd: resolvedCwd, timeoutMs }),
      listGitRemotes({ cwd: resolvedCwd, timeoutMs })
    ])
    branches = Array.isArray(branchInfo?.branches) ? branchInfo.branches : []
    remotes = Array.isArray(remoteInfo?.remotes) ? remoteInfo.remotes : []
  } catch {
  }
  let unstagedDiff = ''
  let unstagedDiffTruncated = false
  let stagedDiff = ''
  let stagedDiffTruncated = false
  try {
    const [unstaged, staged] = await Promise.all([
      getGitDiff({ cwd: resolvedCwd, staged: false, timeoutMs }),
      getGitDiff({ cwd: resolvedCwd, staged: true, timeoutMs })
    ])
    unstagedDiff = String(unstaged?.diff ?? '')
    unstagedDiffTruncated = Boolean(unstaged?.truncated)
    stagedDiff = String(staged?.diff ?? '')
    stagedDiffTruncated = Boolean(staged?.truncated)
  } catch {
  }
  return {
...
export async function stageGitPaths({ cwd = '', paths = [], timeoutMs = 10_000 } = {}) {
  ...
  const status = await getGitStatus({ cwd: result.cwd, timeoutMs })
  return {
    ...status,
    paths: safePaths
  }
...
function parseGitUpstream(upstream) {
  const text = String(upstream ?? '').trim()
  if (!text) return { remote: null, branch: null }
  ...
  return {
    remote: text.slice(0, slash).trim() || null,
    branch: text.slice(slash + 1).trim() || null
  }
}

export async function commitGitChanges({ cwd = '', message, timeoutMs = 20_000 } = {}) {
  const resolvedCwd = resolveWorkspaceListPath(cwd)
  ...
  const status = await getGitStatus({ cwd: resolvedCwd, timeoutMs })
  return {
    ...status,
    message: safeMessage
  }
...
export async function pushGitBranch({
  ...
  const resolvedCwd = resolveWorkspaceListPath(cwd)
  const status = await getGitStatus({ cwd: resolvedCwd, timeoutMs })
  if (status?.notRepo) throw new Error('workspace is not a git repository')
  const safeBranch = String(branch ?? '').trim() || String(status?.branch ?? '').trim()
  if (!safeBranch || safeBranch === 'HEAD') throw new Error('cannot push detached HEAD')

  const upstreamInfo = parseGitUpstream(status?.upstream)
  const safeRemote = String(remote ?? '').trim()
  const remotes = Array.isArray(status?.remotes) ? status.remotes : []
  const remoteName = safeRemote
    || upstreamInfo.remote
    || (remotes.includes('origin') ? 'origin' : (remotes[0] || ''))
  if (!remoteName) throw new Error('no git remote configured')

  const shouldSetUpstream = Boolean(setUpstream) || !upstreamInfo.remote
  const hasExplicitTarget = Boolean(safeRemote) || Boolean(String(branch ?? '').trim())
  const args = ['push']
  if (shouldSetUpstream) {
    args.push('--set-upstream', remoteName, safeBranch)
  } else if (hasExplicitTarget) {
    args.push(remoteName, safeBranch)
  }
  ...
  const nextStatus = await getGitStatus({ cwd: resolvedCwd, timeoutMs })
  return {
    ...nextStatus,
    remote: remoteName,
    branch: safeBranch,
...
export async function listCodexModels({
  ...
  const rpc = new JsonRpcClientClass({
    command: getManagedRunnerToolBinPath('codex'),
    args: ['app-server'],
