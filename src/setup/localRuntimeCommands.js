...
import { buildManagedRunnerToolEnv } from '../lib/runnerTooling.js'
...
export function buildUserServiceInstallOptions(config, {
  ...
  const serviceEnv = buildManagedRunnerToolEnv(env)
  return {
    ...
    environment: {
      ...(serviceEnv.PATH ? { PATH: serviceEnv.PATH } : {}),
      ...(env.CODEX_HOME ? { CODEX_HOME: env.CODEX_HOME } : {}),
      ...(env.ROOTGRID_HOME_DIR ? { ROOTGRID_HOME_DIR: env.ROOTGRID_HOME_DIR } : {}),
      ...(serviceEnv.ROOTGRID_RUNTIME_DIR ? { ROOTGRID_RUNTIME_DIR: serviceEnv.ROOTGRID_RUNTIME_DIR } : {}),
      ...(serviceEnv.ROOTGRID_INSTALL_DIR ? { ROOTGRID_INSTALL_DIR: serviceEnv.ROOTGRID_INSTALL_DIR } : {}),
      ...(serviceEnv.ROOTGRID_CODEX_BIN ? { ROOTGRID_CODEX_BIN: serviceEnv.ROOTGRID_CODEX_BIN } : {}),
      ...(serviceEnv.ROOTGRID_CODE_SERVER_BIN ? { ROOTGRID_CODE_SERVER_BIN: serviceEnv.ROOTGRID_CODE_SERVER_BIN } : {})
    }
  }
}
...
export async function installUserServiceForConfig(config, method = null) {
  ...
  if (chosenMethod === 'systemd-user') {
    ...
    return { method: chosenMethod, unitPath: result.unitPath, linger: result.linger ?? null }
  }
  ...
  if (chosenMethod === 'launchd-user') {
    ...
    return { method: chosenMethod, unitPath: result.unitPath, linger: null }
  }
