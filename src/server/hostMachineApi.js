...
export function createHostMachineApi({
  ...
  return {
    async handle(req, res, url, parts) {
      ...
      if (parts[0] === 'api' && parts[1] === 'machines' && parts[2] && parts[3] === 'tools' && parts.length === 4 && req.method === 'GET') {
        if (!auth.requireAuth(req, res)) return true
        const machineId = parts[2]
        const machine = store.getMachine(machineId)
        if (!machine) {
        ...
        if (!machine?.capabilities?.tools?.enabled) {
          json(res, 501, { error: 'runner does not support tool management yet' })
          return true
        }
        if (typeof getMachineToolsOnRunner !== 'function') {
          json(res, 503, { error: 'runner tool inspection is unavailable' })
          return true
        }

        try {
          const out = await getMachineToolsOnRunner({ machineId })
      ...
      if (parts[0] === 'api' && parts[1] === 'machines' && parts[2] && parts[3] === 'tools' && parts[4] && parts[5] === 'upgrade' && req.method === 'POST') {
        if (!auth.requireAuth(req, res)) return true
        const machineId = parts[2]
        const toolId = String(parts[4] ?? '').trim()
        const machine = store.getMachine(machineId)
        if (!machine) {
          json(res, 404, { error: 'not found' })
          return true
        ...
        if (!toolId) {
          json(res, 400, { error: 'toolId is required' })
          return true
        }
        if (!runnerWs.listConnectedMachineIds().includes(machineId)) {
          json(res, 503, { error: 'runner not connected' })
        ...
        if (typeof upgradeMachineToolOnRunner !== 'function') {
          json(res, 503, { error: 'runner tool upgrades are unavailable' })
          return true
        }

        try {
          const out = await upgradeMachineToolOnRunner({ machineId, toolId })
          json(res, 200, {
            ok: true,
            machineId,
            toolId,
            tool: out?.tool ?? null,
            tools: (out?.tools && typeof out.tools === 'object') ? out.tools : null,
            message: String(out?.message ?? 'Runner tool updated.')
          })
        } catch (err) {
          const code = Number(err?.statusCode) || 500
          json(res, code, { error: String(err?.message ?? err) })
      ...
      if (parts[0] === 'api' && parts[1] === 'machines' && parts[2] && parts[3] === 'tools' && parts[4] && parts[5] === 'auth' && req.method === 'POST') {
        if (!auth.requireAuth(req, res)) return true
        const machineId = parts[2]
        const toolId = String(parts[4] ?? '').trim()
        const machine = store.getMachine(machineId)
        ...
        if (!machine?.capabilities?.tools?.auth) {
          json(res, 501, { error: 'runner does not support web tool auth actions yet' })
          return true
        }
        if (typeof authMachineToolOnRunner !== 'function') {
          json(res, 503, { error: 'runner tool auth actions are unavailable' })
          return true
        }
        ...
        try {
          const out = await authMachineToolOnRunner({
            machineId,
            toolId,
            action,
          ...
          json(res, 200, {
            ...
            tool: out?.tool ?? null,
            tools: (out?.tools && typeof out.tools === 'object') ? out.tools : null,
            message: String(out?.message ?? 'Runner tool auth updated.')
          })
        } catch (err) {
          const code = Number(err?.statusCode) || 500
          json(res, code, { error: String(err?.message ?? err) })
      ...
      if (url.pathname === '/api/git/commit' && req.method === 'POST') {
        ...
        try {
          const out = await gitCommitOnRunner({ machineId: machine.machineId, cwd, message })
          json(res, 200, out)
        } catch (err) {
          const code = Number(err?.statusCode) || 500
          json(res, code, { error: String(err?.message ?? err) })
      ...
      if (url.pathname === '/api/git/push' && req.method === 'POST') {
        ...
        try {
          const out = await gitPushOnRunner({
