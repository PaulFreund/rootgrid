import { normalizeRuntimeSettings } from '../lib/runtimeSettings.js'
...
export function appSettingsRowToRecord(row) {
  if (!row) return null
  const settings = normalizeRuntimeSettings({
    retentionDays: row.retention_days,
    notifications: {
      sseToasts: row.notifications_sse_toasts,
      webPush: row.notifications_web_push,
      sound: Number(row.notifications_sound) === 1
    }
  })
  return {
    ...settings,
