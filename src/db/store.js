...
import {
  ...
  upsertAppSettings as upsertAppSettingsRow,
...
export class Store {
  ...
  #migrateSchema(fromVersion, toVersion) {
    ...
    while (v < toVersion) {
      ...
      if (v === 11) {
        // v11 -> v12: persist per-session service tier (for Codex fast mode).
        try { this.db.exec(`ALTER TABLE sessions ADD COLUMN service_tier TEXT`) } catch { }
      ...
      if (v === 12) {
        // v12 -> v13: host runtime settings move from config.json to SQLite.
        this.db.exec(`
          CREATE TABLE IF NOT EXISTS app_settings (
            singleton_id INTEGER PRIMARY KEY CHECK (singleton_id = 1),
            retention_days INTEGER NOT NULL,
            notifications_sse_toasts TEXT NOT NULL,
            notifications_web_push TEXT NOT NULL,
            notifications_sound INTEGER NOT NULL,
            updated_ms INTEGER NOT NULL
          );
        `)
   ...
   *   options?: { model?: string, reasoningEffort?: string, serviceTier?: string, approvalPolicy?: string, sandbox?: string }|null
   * }} input
   */
  ...
  getAppSettings() {
    return getAppSettingsRow(this.db)
  }

  upsertAppSettings({ retentionDays, notifications }) {
    upsertAppSettingsRow(this.db, { retentionDays, notifications })
  }
