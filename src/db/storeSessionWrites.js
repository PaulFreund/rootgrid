...
export function normalizeSessionOptionValues(options = null) {
  return {
    ...
    serviceTier: trimStringOrNull(options?.serviceTier),
...
export function buildUpdateSessionStatement({
  ...
  if (serviceTier !== undefined) { sets.push('service_tier=?'); params.push(serviceTier) }
  ...
  if (type === 'session.status') {
    ...
    if (status && typeof status === 'string') {
      ...
      if (status === 'running') {
        sets.push('turn_state=?')
        params.push('running')
      } else if (status === 'stopping' || status === 'exited' || status === 'failed') {
        sets.push('turn_state=?')
        params.push('idle')
      }
    }
