...
test('createSessionFromDraft clears the new-thread draft via callback after a successful send', async () => {
  const selectedSessionId = ref(null)
  const clearedDrafts = []
  ...
  const { createSessionFromDraft } = createComposerSessionActions({
    ...
    clearDraftForSession: (sessionId) => clearedDrafts.push(sessionId),
    defaults: reactive({
      cwd: '/workspace/rootgrid',
      machineId: '',
      model: '',
    ...
    sendError: ref(''),
    messageDraft: ref('draft'),
    sending: ref(false),
    selectedSession: ref(null),
    selectedSessionId
  })

  const sessionId = await createSessionFromDraft('draft')

  assert.equal(sessionId, 'session-1')
  assert.deepEqual(clearedDrafts, [null])
  assert.equal(selectedSessionId.value, 'session-1')
})
...
test('submit clears the source session draft by session id after sending', async () => {
  const clearedDrafts = []
  ...
  assert.deepEqual(clearedDrafts, ['session-1'])
  assert.equal(selectedSessionId.value, 'session-2')
})
...
test('queued prompt actions reconcile stale missing prompts instead of surfacing not found errors', async () => {
  const sessionStores = reactive(new Map([['session-1', createSessionStoreState()]]))
  sessionStores.get('session-1').queuedPrompts.push({
    id: 'queued-1',
    promptId: 'queued-1',
    ...
    apiFetch: async (path, init = {}) => {
      ...
      if (path === '/api/sessions/session-1/queued-prompts/queued-1' && init.method === 'DELETE') {
        return {
          ...
          status: 404,
          async json() {
            return { error: 'queued prompt not found' }
          }
        }
      }
