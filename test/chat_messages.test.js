...
test('parseUnifiedDiff marks whitespace-only line changes separately from substantive ones', () => {
  const diff = [
    'diff --git a/src/a.txt b/src/a.txt',
    '--- a/src/a.txt',
    '+++ b/src/a.txt',
  ...
  assert.equal(file.lines[4].changeKind, 'whitespace')
  assert.equal(file.lines[5].changeKind, 'whitespace')
  assert.equal(file.lines[6].changeKind, 'substantive')
  assert.equal(file.lines[7].changeKind, 'substantive')
})
...
test('buildChatMessages nests active-turn command stderr into the thought timeline', () => {
  const store = createSessionStoreState()
  store.backgroundExpandedByTurnId.set('turn-stderr', true)
  ...
  store.events.push(
    ...
    },
    {
      ...
      payload: {
        stream: 'stderr',
        text: '2026-05-16T08:55:50Z ERROR codex_core::tools::router: CreateProcess rejected command\n'
      }
    },
    {
      eventId: 'turn-done',
      ...
      tsMs: 3_000,
      type: 'turn.completed',
      payload: { turnId: 'turn-stderr', status: 'completed' }
    }
  )

  const messages = buildChatMessages(store)
  assert.equal(messages.length, 1)
  assert.equal(messages[0].stepKind, 'background')
  assert.equal(messages[0].timeline.some((entry) => entry.kind === 'stderr'), true)
  assert.match(String(messages[0].timeline.find((entry) => entry.kind === 'stderr')?.text ?? ''), /CreateProcess rejected command/)
})

test('buildChatMessages keeps synthesized codex failure stderr as a standalone system error', () => {
  const store = createSessionStoreState()
  store.backgroundExpandedByTurnId.set('turn-fail', true)
  ...
  store.events.push(
    ...
    },
    {
      ...
      payload: {
        stream: 'stderr',
        text: '[codex] Turn failed: process exited 1\n'
      }
    }
  )

  const messages = buildChatMessages(store)
  assert.equal(messages.some((message) => message.role === 'system' && message.stream === 'stderr' && message.tone === 'error'), true)
})
