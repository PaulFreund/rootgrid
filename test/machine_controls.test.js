...
test('upgradeMachines starts all requested upgrades and aggregates failures', async () => {
  const requests = []
  const machineUpgradeWorkingIds = ref([])
  const machineUpgradeError = ref('')

  const { upgradeMachines } = createMachineControlActions({
    apiFetch: async (path, init = {}) => {
      requests.push({ path, init, workingIds: [...machineUpgradeWorkingIds.value] })
      if (path === '/api/machines/machine-fail/upgrade') {
        return {
          ok: false,
          status: 409,
          async json() {
            return { error: 'finish 1 running session before upgrading this runner' }
          }
        }
      }
      return {
    ...
    machineUpgradeError,
    appSettings: reactive({ appVersion: '1.2.3' }),
    getMachineLabel: (machineId) => machineId === 'machine-fail' ? 'Failing runner' : 'Healthy runner',
    ideError: ref(''),
    ideStarting: ref(false)
  })

  const ok = await upgradeMachines(['machine-ok', 'machine-fail'])

  assert.equal(ok, false)
  assert.equal(machineUpgradeWorkingIds.value.length, 0)
  assert.match(machineUpgradeError.value, /Failing runner: finish 1 running session/)
  assert.deepEqual(requests.map((req) => req.path), [
    '/api/machines/machine-ok/upgrade',
    '/api/machines/machine-fail/upgrade'
  ])
})
