...
test('createAppSettingsActions login loads settings and connects SSE on success', async () => {
  ...
  assert.equal(soundNotificationsDraft.value, true)
...
test('createAppSettingsActions starts host self-update and keeps the working flag set while restart is pending', async () => {
  ...
  const { startHostSelfUpdate } = createAppSettingsActions({
    ...
    soundNotificationsDraft: ref(false),
...
test('createAppSettingsActions reloads the page after the updated host comes back', async () => {
  const hostSelfUpdateWorking = ref(false)
  const hostSelfUpdateError = ref('')
  const hostSelfUpdateStatus = ref('')
  const timers = []
  ...
  const { startHostSelfUpdate } = createAppSettingsActions({
    apiFetch: async (path, init = {}) => {
        ...
        return {
          ...
          async json() {
            return {
              ...
              selfUpdate: { awaitingRestart: true }
            }
          }
        }
      }
      if (path === '/api/settings') {
        settingsChecks += 1
        const awaitingRestart = settingsChecks < 2
        return {
          ok: true,
          status: 200,
          async json() {
  ...
  assert.equal(timers.length, 1)
  assert.deepEqual(apiCalls.map((call) => call.path), ['/api/host/self-update'])

  await timers.shift()()
  assert.equal(reloads, 0)
  assert.equal(timers.length, 1)
  assert.deepEqual(apiCalls.map((call) => call.path), ['/api/host/self-update', '/api/settings'])

  await timers.shift()()
  assert.equal(reloads, 1)
  assert.equal(hostSelfUpdateError.value, '')
  assert.equal(hostSelfUpdateStatus.value, 'Host restarted. Reloading…')
  assert.deepEqual(apiCalls.map((call) => call.path), ['/api/host/self-update', '/api/settings', '/api/settings'])
})
