...
test('session metadata routes rename, patch options, archive, unarchive, and delete sessions', async (t) => {
  ...
  assert.equal(options.data?.session?.serviceTier, 'fast')
...
test('workspace helper routes proxy fs, git, and terminal requests to the runner', async (t) => {
  ...
  runner.on('message', (buf) => {
    ...
    if (msg.type === 'machine.tools.upgrade') {
      runner.send(JSON.stringify({
        v: 1,
  ...
  assert.match(gitData.stagedDiff ?? '', /src\/app\.js/)
  ...
  const tools = await apiJson({
    port,
    path: `/api/machines/${machineId}/tools`,
    cookie
  })
  assert.equal(tools.res.status, 200)
  assert.equal(tools.data?.tools?.codex?.version, 'codex-cli 1.2.3')

  const upgraded = await apiJson({
    port,
    path: `/api/machines/${machineId}/tools/codex/upgrade`,
    method: 'POST',
    cookie,
    body: {}
  })
  assert.equal(upgraded.res.status, 200)
  assert.equal(upgraded.data?.tool?.version, 'codex-cli 9.9.9')
})
...
test('system settings and push subscription routes persist DB-backed settings + subscriptions', async (t) => {
  const svc = await startHostProcess({ cwd: process.cwd() })
  t.after(async () => {
    await svc.stop()
  })
  ...
  assert.equal(update.data?.notifications?.sound, true)

  const savedConfig = JSON.parse(await readFile(configPath, 'utf8'))
  assert.equal(savedConfig.retentionDays, svc.config.retentionDays)
  assert.equal(savedConfig.notifications?.sseToasts, svc.config.notifications.sseToasts)
  assert.equal(savedConfig.notifications?.webPush, svc.config.notifications.webPush)
  assert.equal(savedConfig.notifications?.sound, svc.config.notifications.sound)
  ...
  const persistedSettings = store.getAppSettings()
  assert.deepEqual(persistedSettings, {
    ...
    updatedMs: persistedSettings?.updatedMs
  })
  assert.ok(Number(persistedSettings?.updatedMs) > 0)
