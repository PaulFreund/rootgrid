...
test('buildCreateSessionRow fills the expected session defaults', () => {
  const row = buildCreateSessionRow({
    ...
    options: { model: 'gpt-5', serviceTier: 'fast' },
    now: 123
  })
...
test('buildUpdateSessionStatement emits only changed columns plus updated timestamp', () => {
  ...
  assert.deepEqual(out, {
    sets: ['status=?', 'preview=?', 'service_tier=?', 'sandbox_mode=?', 'updated_ms=?'],
    params: ['running', 'hello', 'fast', 'danger-full-access', 999]
  })
})
...
test('buildDerivedSessionEventUpdate captures preview/title/status/approval derived metadata', () => {
  assert.deepEqual(buildDerivedSessionEventUpdate({
    ...
    sets: ['updated_ms=?', 'last_seq=?', 'turn_state=?', 'preview=?', "title = CASE WHEN COALESCE(title_source, 'auto')='user' THEN title WHEN title IS NULL OR title='' THEN ? ELSE title END"],
    params: [10, 1, 'running', 'first message', 'first message']
  })
  ...
  assert.deepEqual(buildDerivedSessionEventUpdate({
    ...
    sets: ['updated_ms=?', 'last_seq=?', 'status=?', 'turn_state=?', 'codex_thread_id=?', 'preview=?', "title = CASE WHEN COALESCE(title_source, 'auto')='user' THEN title ELSE ? END"],
    params: [20, 2, 'running', 'running', 'thread-1', 'Summarize this repository', 'Repo overview']
  })
