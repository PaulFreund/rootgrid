...
test('buildRunnerInstallScript generates a self-contained bootstrap script', () => {
  ...
  assert.match(script, /rootgrid_install_managed_codex/)
  assert.match(script, /mktemp -d "\$cache_parent\/codex-npm\.XXXXXX"/)
  assert.match(script, /npm install --global --prefix "\$ROOTGRID_CODEX_PREFIX" --cache "\$npm_cache" @openai\/codex/)
  ...
  assert.match(script, /rootgrid_install_managed_code_server/)
  assert.match(script, /mktemp -d "\$cache_parent\/code-server\.XXXXXX"/)
  assert.match(script, /XDG_CACHE_HOME="\$install_cache"/)
