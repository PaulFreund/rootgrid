...
test('approval and sandbox compatibility helpers prefer current variants before legacy fallbacks', () => {
  ...
  assert.deepEqual(serviceTierCandidates('fast'), ['fast'])
  assert.deepEqual(serviceTierCandidates('Flex'), ['flex'])
...
test('buildResumeThreadAttempts includes threadId/id fallbacks plus service tier and reasoning variants', () => {
  const attempts = buildResumeThreadAttempts({
    ...
    serviceTiers: serviceTierCandidates('fast'),
  ...
  assert.ok(attempts.some((params) => {
    return params.id === 'thread-1' && params.service_tier === 'fast'
  }))
  assert.ok(attempts.some((params) => {
    return params.threadId === 'thread-1' && params.reasoning?.effort === 'medium'
  }))
  assert.ok(attempts.some((params) => {
    return params.threadId === 'thread-1' && !('serviceTier' in params) && !('service_tier' in params)
  }))
})
