export function createSessionStoreState() {
  return {
    ...
    restoredPromptError: '',
    codexAuthIssue: null,
    codexMissingIssue: null,
    codexMissingPromptDismissed: false,
    workspacePaneOpen: false,
    workspacePaneTab: 'code',
...
export function sessionHasWorkingTurn(session) {
  return (session?.turnState === 'running') || (session?.status === 'starting')
}

export function sessionHasUnread(session) {
  return Number(session?.lastSeq ?? 0) > Number(session?.lastReadSeq ?? 0)
}

export function sessionIndicator(session) {
  const pending = Number(session?.pendingApprovals ?? 0)
  if (pending > 0) return 'red'
  if (sessionHasWorkingTurn(session)) return 'orange'
  if (sessionHasUnread(session)) return 'blue'
  return 'green'
}
...
export function sessionListAccentTone(session) {
  ...
  if (sessionHasWorkingTurn(session)) return 'default'
  if (sessionHasUnread(session)) return 'blue'
  return 'default'
}
...
export function sessionListAccentClass(session, { selected = false } = {}) {
  ...
  if (tone === 'red') return selected ? 'bg-red-600 text-white shadow-sm' : 'bg-red-500 text-white hover:bg-red-500'
  if (tone === 'blue') return selected ? 'bg-sky-600 text-white shadow-sm' : 'bg-sky-500 text-white hover:bg-sky-500'
  return selected ? 'bg-[#d8d7ce] text-slate-900 shadow-sm ring-1 ring-black/[0.05]' : 'hover:bg-black/[0.035]'
}
