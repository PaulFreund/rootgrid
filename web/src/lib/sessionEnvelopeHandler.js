...
export function createSessionEnvelopeHandler({
  ...
  return function handleEnvelope(env) {
    ...
    if (env.type === 'toast') {
      ...
      try { playToastSound?.(toast) } catch { }
    ...
    if (env.type === 'session.error') {
      try {
        onSessionError?.({
          env,
          sessionId,
          machineId: env.scope?.machineId ?? env.payload?.machineId ?? null,
    ...
    if (session) {
      ...
      if (env.type === 'session.status' && env.payload?.status) {
        ...
        if (env.payload.status === 'running') session.turnState = 'running'
        if (env.payload.status === 'stopping' || env.payload.status === 'exited' || env.payload.status === 'failed') {
    ...
    if (env.type === 'thread.tokenUsage.updated' || env.type === 'token.count' || env.type === 'account.rateLimits.updated') {
