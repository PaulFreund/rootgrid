...
import { parseUnifiedDiff, summarizeUnifiedDiff } from './unifiedDiff.js'

const COMMAND_ACTIONS_CACHE = new WeakMap()
const CHAT_MESSAGES_CACHE = new WeakMap()

export { parseUnifiedDiff, summarizeUnifiedDiff }
...
function shouldNestTurnStderr(text) {
  const value = String(text ?? '').trim()
  if (!value) return false
  // Keep Rootgrid/Codex synthesized failure summaries as standalone errors.
  if (value.startsWith('[codex]')) return false
  return true
}

function buildTurnBackgroundTimeline({ exploreCalls, reasoningState, toolCalls, reasoningChunks, commentaryChunks, stderrChunks, diffEvents, errors }) {
  ...
  const rawStderrChunks = Array.isArray(stderrChunks) ? stderrChunks : []
  ...
  for (const chunk of rawStderrChunks) {
    if (!chunk) continue
    const text = String(chunk.text ?? '')
    if (!text.trim()) continue
    const seq = Number(chunk.seq ?? NaN)
    ...
    items.push({
      kind: 'stderrChunk',
      id: `stderr-live-${String(chunk.id ?? items.length + 1)}`,
      seq: Number.isFinite(seq) ? seq : null,
      tsMs: Number.isFinite(tsMs) ? tsMs : null,
      text
    })
  ...
  for (const item of ordered) {
    ...
    if (prev?.kind === 'stderrChunk' && item.kind === 'stderrChunk') {
      prev.text += item.text
      continue
    }
...
function buildChatMessagesSlow(store) {
  ...
  for (const event of events) {
    ...
    if (event.type === 'session.output') {
      ...
      } else if (stream === 'stderr' && !event.payload?.itemId) {
        const turnId = String(activeTurnId ?? '').trim()
        if (turnId && shouldNestTurnStderr(text)) {
          const state = ensureTurnBackgroundMessage(turnId)
          if (state) {
            state.stderrChunks.push({
              id: event.eventId,
              text: String(text ?? ''),
              seq: event.seq ?? null,
              tsMs: event.tsMs ?? null
  ...
  const filtered = msgs.filter((msg) => {
    ...
    const hasNestedStderr = Array.isArray(state.stderrChunks) && state.stderrChunks.length > 0
    ...
    if (!isCurrentTurn && !hasReasoning && !hasCommentary && !hasNestedStderr && !hasErrors && !explore.files && !explore.searches && !explore.lists && !explore.active && !toolCalls.length && !state.diffEvents.length) {
      return false
    }
    ...
    msg.timeline = expanded
      ? buildTurnBackgroundTimeline({
          ...
          stderrChunks: state.stderrChunks,
          errors: state.errors
        })
      : null
    return true
