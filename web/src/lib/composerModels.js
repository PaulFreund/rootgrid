...
import { detectCodexMissingIssueText } from './codexAuthUi.js'
...
export function resolveComposerModelFallbackLabel({
  currentModel = '',
  defaultModel = null,
  defaultLabel = ''
} = {}) {
  ...
  if (!current) return ''
  const defaultId = modelIdFor(defaultModel)
  if (!defaultId || current !== defaultId) return ''
  return String(defaultLabel ?? '').trim() || modelLabel(defaultModel) || ''
}
...
export function createComposerModelSettings({
  ...
  const composerServiceTierValue = computed(() => {
    return String((selectedSession.value ? selectedSession.value.serviceTier : defaults.serviceTier) ?? '')
  })
  ...
  async function loadModelCatalog({ force = false } = {}) {
    ...
    try {
      ...
      const missing = detectCodexMissingIssueText(message)
      modelCatalogError.value = missing?.message ?? message
    } finally {
  ...
  function rememberComposerDefaults(patch) {
    ...
    if (Object.prototype.hasOwnProperty.call(patch, 'serviceTier')) {
      defaults.serviceTier = String(patch.serviceTier ?? '').trim()
    }
  ...
  const composerFastMode = computed({
    get() {
      return normalizeServiceTier(composerServiceTierValue.value) === 'fast'
    },
    set(value) {
      const next = value ? 'fast' : ''
      const current = normalizeServiceTier(composerServiceTierValue.value)
      if (next === current) return
      rememberComposerDefaults({ serviceTier: next })
      if (selectedSession.value) patchSelectedSessionOptions({ serviceTier: next || null })
    }
  })
