...
import { Archive, ArrowDown, ArrowLeft, ArrowUp, CheckCircle2, ChevronDown, ChevronUp, Circle, Code, Copy, FileText, FolderClosed, GitBranch, Keyboard, Loader2, Menu, MoreHorizontal, Pencil, Plus, RotateCw, Settings, Sidebar, Square, Terminal, Trash2, X, Zap } from 'lucide-vue-next'

import MarkdownView from './components/MarkdownView.vue'
import CodexAuthPanel from './components/CodexAuthPanel.vue'
...
import {
  ...
  machineSupportsToolAuth as machineSupportsToolAuthAt,
  machineSupportsToolManagement as machineSupportsToolManagementAt,
...
import {
  createSessionReadGate
} from './lib/sessionReadGate.js'
import {
...
let machineToolAuthPollTimer = null
...
const LAST_RATE_LIMITS_STORAGE_KEY = 'rootgrid.lastRateLimits'
...
const SESSION_LIST_SESSION_SLOT_HEIGHT = 72
const COMPACT_SIDEBAR_SESSION_SLOT_HEIGHT = SESSION_LIST_SESSION_SLOT_HEIGHT
const COMPACT_SIDEBAR_GROUP_SLOT_HEIGHT = 28
const compactSidebarSessionSlotStyle = Object.freeze({
  height: `${COMPACT_SIDEBAR_SESSION_SLOT_HEIGHT}px`
})
const expandedSidebarSessionSlotStyle = Object.freeze({
  height: `${SESSION_LIST_SESSION_SLOT_HEIGHT}px`
})
...
const lastKnownRateLimits = ref(readStoredLastRateLimits())
...
const machinesForSelect = computed(() => {
  return sortMachinesForSettings(machines.value, sessions.value, nowMs.value)
})
...
const cleanSessionRow = computed(() => {
  const sid = String(cleanSessionId.value ?? '').trim()
  if (!sid) return null
  return sessionRowsById.get(sid)
    ?? sessions.value.find((session) => session?.sessionId === sid)
    ?? archivedSessions.value.find((session) => session?.sessionId === sid)
    ?? null
})
...
const machineCodexIssueToastSeen = reactive({})
...
const soundNotificationsDraft = ref(false)
...
const hostSelfUpdateHasRepo = computed(() => {
  const repo = String(hostSelfUpdateInfo.value?.repo ?? '').trim()
  return Boolean(repo && repo !== '—')
})
const hostSelfUpdateEnabled = computed(() => Boolean(hostSelfUpdateInfo.value?.enabled && hostSelfUpdateHasRepo.value))
const hostSelfUpdateDisabledReason = computed(() => {
  if (hostSelfUpdateEnabled.value) return ''
  if (!hostSelfUpdateHasRepo.value) return 'missing-repo'
  if (hostSelfUpdateInfo.value?.enabled === false) return 'disabled'
  return 'unavailable'
})
const hostSelfUpdateInstalledInfo = computed(() => {
  const installed = (hostSelfUpdateInfo.value?.installed && typeof hostSelfUpdateInfo.value.installed === 'object')
    ? hostSelfUpdateInfo.value.installed
    : null
  return {
    version: String(installed?.version ?? '').trim() || null,
    versionHash: String(installed?.versionHash ?? '').trim() || null,
    releaseId: String(installed?.releaseId ?? '').trim() || null,
    bundleSha256: String(installed?.bundleSha256 ?? '').trim() || null,
    source: String(installed?.source ?? '').trim() || null,
    createdAtMs: Number.isFinite(Number(installed?.createdAtMs)) ? Number(installed.createdAtMs) : null
  }
})
const hostSelfUpdateLatestInfo = computed(() => hostSelfUpdateInfo.value?.latest ?? null)
const hostSelfUpdateUpdateAvailable = computed(() => hostSelfUpdateInfo.value?.updateAvailable === true)
const hostSelfUpdateUpToDate = computed(() => hostSelfUpdateInfo.value?.upToDate === true)
const hostSelfUpdateLatestCheckError = computed(() => String(hostSelfUpdateLatestInfo.value?.error ?? '').trim())

function hostSelfUpdateReleaseVersion(release, fallback = 'unknown') {
  const version = String(release?.version ?? '').trim()
  return version || fallback
}

function hostSelfUpdateReleaseHash(release) {
  const bundleSha256 = String(release?.bundleSha256 ?? '').trim()
  if (bundleSha256) return bundleSha256
  const versionHash = String(release?.versionHash ?? '').trim()
  return versionHash || null
}

function hostSelfUpdateReleaseHashShort(release) {
  const hash = hostSelfUpdateReleaseHash(release)
  if (!hash) return '—'
...
const workspaceGitCommitWorking = ref(false)
const workspaceGitPushWorking = ref(false)
const workspaceGitBranchDraft = ref('')
const workspaceGitCommitMessage = ref('')
...
const WORKSPACE_PANE_TAB_SET = new Set(['code', 'files', 'git', 'terminal'])
...
const machineUpgradeWorkingIds = ref([])
...
let readInteractionHandler = null
const composerMachinePopoverClose = createHoverCloseController()
const sessionReadGate = createSessionReadGate()

function createComposerDraftRecord() {
  return reactive({
    text: '',
    attachments: []
  })
...
function getComposerDraftRecord(sessionId = selectedSessionId.value) {
  return getOrCreateComposerDraft(
    composerDrafts,
    composerDraftKeyForSession(sessionId),
    { createDraft: createComposerDraftRecord }
  )
}

function clearComposerDraftForSession(sessionId = selectedSessionId.value) {
  const draft = getComposerDraftRecord(sessionId)
  draft.text = ''
  clearComposerAttachmentList(draft.attachments)
  draft.attachments = []
}

const messageDraft = computed({
  get() {
    return String(getComposerDraftRecord().text ?? '')
  },
  set(value) {
    getComposerDraftRecord().text = String(value ?? '')
  }
})

const attachments = computed({
  get() {
    return getComposerDraftRecord().attachments
  },
  set(value) {
    getComposerDraftRecord().attachments = Array.isArray(value) ? value : []
  }
})
...
function normalizeWorkspacePaneTab(value) {
  const tab = String(value ?? '').trim()
  return WORKSPACE_PANE_TAB_SET.has(tab) ? tab : 'code'
}

function persistWorkspacePaneState(sessionId = selectedSessionId.value) {
  const sid = String(sessionId ?? '').trim()
  if (!sid) return false
  const store = getSessionStore(sid)
  store.workspacePaneOpen = Boolean(workspacePaneOpen.value)
  store.workspacePaneTab = normalizeWorkspacePaneTab(workspacePaneTab.value)
  return true
}

function restoreWorkspacePaneState(sessionId = selectedSessionId.value) {
  const sid = String(sessionId ?? '').trim()
  if (!sid) {
  ...
  const store = getSessionStore(sid)
  workspacePaneOpen.value = Boolean(store.workspacePaneOpen)
  workspacePaneTab.value = normalizeWorkspacePaneTab(store.workspacePaneTab)
  return store
}

function upsertSessionRow(value) {
  upsertSessionRowInList(sessions.value, value, sessionRowsById)
  syncSessionReadRequirement(sessionRowsById.get(String(value?.sessionId ?? '').trim()) ?? value)
}
...
function replaceAllSessionRows(rows) {
  ...
  for (const session of sessions.value) syncSessionReadRequirement(session)
}
...
function removeSessionRow(sessionId) {
  deleteComposerDraft(
    composerDrafts,
    composerDraftKeyForSession(sessionId),
    { clearComposerAttachments: clearComposerAttachmentList }
  )
  ...
  sessionReadGate.clearSession(sessionId)
}

function syncSessionReadRequirement(session) {
  const sid = String(session?.sessionId ?? '').trim()
  if (!sid) return false
  return sessionReadGate.syncSessionUnread(sid, sessionHasUnread(session))
}
...
function readStoredLastRateLimits() {
  try {
    if (typeof localStorage === 'undefined') return null
    const raw = localStorage.getItem(LAST_RATE_LIMITS_STORAGE_KEY)
    if (!raw) return null
...
function persistLastRateLimits(value) {
  try {
    if (typeof localStorage === 'undefined') return value ?? null
    if (!value || typeof value !== 'object') {
      localStorage.removeItem(LAST_RATE_LIMITS_STORAGE_KEY)
      return null
    }
    localStorage.setItem(LAST_RATE_LIMITS_STORAGE_KEY, JSON.stringify(value))
  } catch {
  }
  return value ?? null
}
...
function showInlineToast({
  ...
  if (toasts.value.some((toast) => String(toast?.id ?? '').trim() === toastId)) return false
  toasts.value.push({
    id: toastId,
    level,
    title: String(title ?? '').trim() || 'Rootgrid',
    message: String(message ?? '').trim(),
    sessionId,
    stickyUntilVisible: false
  })
  scheduleDismissToast(toastId, ms)
  return true
}
...
const notificationSoundPlayer = createNotificationSoundPlayer({
  isEnabled: () => appSettings.notifications?.sound === true
})

function playToastSound() {
  return notificationSoundPlayer.play()
}
...
function maybeUpdateTokenUsage(sessionId, payload) {
  ...
  try {
    const rateLimits = tokenUsageBySessionId.get(sessionId)?.rateLimits ?? null
    if (rateLimits) lastKnownRateLimits.value = persistLastRateLimits(rateLimits)
  } catch {
  }
}
...
function machineSupportsToolManagement(m) {
  return machineSupportsToolManagementAt(m)
}

function machineSupportsToolAuth(m) {
  return machineSupportsToolAuthAt(m)
}

function machineCodexSupportMessage(machine) {
  if (machineSupportsToolAuth(machine)) return ''
  if (machineSupportsToolManagement(machine)) {
    return 'This runner needs an update before Rootgrid can start ChatGPT sign-in from the web UI. Update the runner, or run codex logout followed by codex login --device-auth on the runner shell.'
  }
  return 'This runner is too old for Rootgrid-managed Codex sign-in. Update the runner first, or run codex logout followed by codex login --device-auth on the runner shell.'
}
...
function machineToolsFor(machineId) {
  const mid = String(machineId ?? '').trim()
  const tools = machineToolsByMachineId[mid]
  return [
    tools?.codex ?? null,
    tools?.codeServer ?? null
  ].filter(Boolean)
}

function machineCodexTool(machineId) {
  const mid = String(machineId ?? '').trim()
  return machineToolsByMachineId[mid]?.codex ?? null
}

function setSessionCodexMissingIssue(sessionId, issue, { reopen = false } = {}) {
  const sid = String(sessionId ?? '').trim()
  if (!sid) return
  const store = getSessionStore(sid)
  if (!store) return

  const nextIssue = (issue && typeof issue === 'object')
    ? {
        code: String(issue.code ?? '').trim() || 'codex_missing',
        message: String(issue.message ?? '').trim() || 'Managed Codex is missing on this runner. Install it to use chat sessions and model listing.',
        machineId: String(issue.machineId ?? '').trim() || null,
        source: String(issue.source ?? '').trim() || 'unknown',
        detectedAtMs: Number.isFinite(Number(issue.detectedAtMs)) ? Number(issue.detectedAtMs) : Date.now()
      }
    : null

  const current = store.codexMissingIssue
  const sameIssue = Boolean(
    current
    && nextIssue
    && String(current.code ?? '') === String(nextIssue.code ?? '')
    && String(current.message ?? '') === String(nextIssue.message ?? '')
    && String(current.machineId ?? '') === String(nextIssue.machineId ?? '')
    && String(current.source ?? '') === String(nextIssue.source ?? '')
  )

  store.codexMissingIssue = nextIssue
  if (nextIssue && (!sameIssue || reopen)) {
    store.codexMissingPromptDismissed = false
  }
}

function clearSessionCodexMissingIssue(sessionId) {
  const sid = String(sessionId ?? '').trim()
  if (!sid) return
  const store = getSessionStore(sid)
  if (!store) return
  store.codexMissingIssue = null
  store.codexMissingPromptDismissed = false
}

function dismissSelectedSessionCodexInstallPrompt() {
  if (!selectedStore.value) return
  selectedStore.value.codexMissingPromptDismissed = true
}

function machineToolsLoading(machineId) {
  const mid = String(machineId ?? '').trim()
  return Boolean(machineToolsLoadingByMachineId[mid])
}

function machineToolsError(machineId) {
  const mid = String(machineId ?? '').trim()
  return String(machineToolsErrorByMachineId[mid] ?? '').trim()
}

function machineToolActionLabel(tool) {
  if (!tool?.installed) return 'Install'
  if (tool?.source === 'external') return 'Install managed'
  return 'Reinstall'
}

function machineToolWorking(machineId, toolId) {
  const mid = String(machineId ?? '').trim()
  const tid = String(toolId ?? '').trim()
  if (!mid || !tid) return false
  return Boolean(
    machineToolUpgradeWorking[`${mid}:${tid}`]
    || machineToolAuthWorking[`${mid}:${tid}`]
    || machineToolsByMachineId[mid]?.[tid]?.auth?.flow?.active
    || machineToolsByMachineId[mid]?.[tid]?.working
  )
}

function machineToolErrorText(machineId, toolId) {
  const mid = String(machineId ?? '').trim()
  const tid = String(toolId ?? '').trim()
  if (!mid || !tid) return ''
  return String(machineToolUpgradeError[`${mid}:${tid}`] ?? machineToolsByMachineId[mid]?.[tid]?.lastError ?? '').trim()
}

function machineToolStatusText(machineId, toolId) {
  const mid = String(machineId ?? '').trim()
  const tid = String(toolId ?? '').trim()
  if (!mid || !tid) return ''
  return String(machineToolUpgradeStatus[`${mid}:${tid}`] ?? '').trim()
}

function machineToolAuthBusy(machineId, toolId) {
  const mid = String(machineId ?? '').trim()
  const tid = String(toolId ?? '').trim()
  if (!mid || !tid) return false
  return Boolean(machineToolAuthWorking[`${mid}:${tid}`] || machineToolsByMachineId[mid]?.[tid]?.working)
}

function machineToolAuthErrorText(machineId, toolId) {
  const mid = String(machineId ?? '').trim()
  const tid = String(toolId ?? '').trim()
  if (!mid || !tid) return ''
  return String(machineToolAuthError[`${mid}:${tid}`] ?? '').trim()
}

function machineToolAuthStatusText(machineId, toolId) {
  const mid = String(machineId ?? '').trim()
  const tid = String(toolId ?? '').trim()
  if (!mid || !tid) return ''
  return String(machineToolAuthStatus[`${mid}:${tid}`] ?? '').trim()
}

function clearMachineCodexIssueToastSeen(machineId) {
  const mid = String(machineId ?? '').trim()
  if (!mid) return
  for (const key of Object.keys(machineCodexIssueToastSeen)) {
    if (key.startsWith(`${mid}:`)) delete machineCodexIssueToastSeen[key]
  }
}

async function refreshMachineToolAuth(machineId, toolId) {
  const ok = await authMachineTool(machineId, toolId, 'refresh')
  if (ok && toolId === 'codex') clearMachineCodexIssueToastSeen(machineId)
  return ok
}

async function logoutMachineTool(machineId, toolId) {
  const mid = String(machineId ?? '').trim()
  const tid = String(toolId ?? '').trim()
  const ok = await authMachineTool(mid, tid, 'logout')
  if (ok && tid === 'codex') clearMachineCodexIssueToastSeen(mid)
  return ok
}

async function startMachineToolDeviceAuth(machineId, toolId) {
  const ok = await authMachineTool(machineId, toolId, 'startDeviceAuth')
  if (ok && toolId === 'codex') clearMachineCodexIssueToastSeen(machineId)
  return ok
}

async function cancelMachineToolDeviceAuth(machineId, toolId) {
  return await authMachineTool(machineId, toolId, 'cancelDeviceAuth')
}

async function installSelectedSessionCodex() {
  const machineId = String(selectedSessionMachineId.value ?? '').trim()
  if (!machineId) return false
  const ok = await upgradeMachineTool(machineId, 'codex')
  if (!ok) return false
  clearSessionCodexMissingIssue(selectedSessionId.value)
  loadMachineTools(machineId, { force: true }).catch(() => {})
  loadModelCatalog({ force: true }).catch(() => {})
  return true
}
...
function machineUpgradeWorking(machineId) {
  const mid = String(machineId ?? '').trim()
  return Boolean(mid && Array.isArray(machineUpgradeWorkingIds.value) && machineUpgradeWorkingIds.value.includes(mid))
}
...
function machineUpgradeAvailable(m) {
  return Boolean(
    machineIsOnline(m)
    && ((machineHasVersionMismatch(m) && machineSupportsWebUpgrade(m)) || machineHasUnknownVersion(m))
  )
}

function machineCanStartUpgrade(m) {
  return Boolean(
    machineUpgradeAvailable(m)
    && !machineUpgradeBusy(m)
    && !machineUpgradeWorking(m?.machineId)
...
function scheduleMarkRead(sessionId) {
  ...
  const session = sessionRowsById.get(sid) ?? null
  if (!sessionHasUnread(session)) {
    sessionReadGate.clearSession(sid)
    clearScheduledMarkRead()
    return
  }
  if (!sessionReadGate.canMarkRead(sid)) {
    clearScheduledMarkRead()
    return
  }
  clearScheduledMarkRead()
  markReadTimer = setTimeout(() => {
    markReadTimer = null
    const latest = sessionRowsById.get(sid) ?? null
    if (!sessionHasUnread(latest)) {
      sessionReadGate.clearSession(sid)
      return
    }
    if (!sessionReadGate.canMarkRead(sid)) return
    markSessionRead(sid)
  }, 500)
}
...
function applyRestoredQueuedPromptToComposer({ sessionId, prompt = null, error = '' } = {}) {
  ...
  const draft = getComposerDraftRecord(sid)
  const nextError = String(error ?? store?.restoredPromptError ?? '').trim()
  draft.text = String(restoredPrompt?.text ?? '')
  queuedPromptEditingId.value = null
  queuedPromptEditDraft.value = ''
  clearComposerAttachmentList(draft.attachments)
  draft.attachments = buildComposerAttachmentRefs(restoredPrompt?.attachments, sid)
  if (String(selectedSessionId.value ?? '').trim() === sid) {
    if (nextError) sendError.value = nextError
  }
  if (store && String(selectedSessionId.value ?? '').trim() === sid) {
...
const baseHandleEnvelope = createSessionEnvelopeHandler({
  ...
  onSessionError: ({ sessionId, machineId, payload }) => {
    const missingCodex = String(payload?.code ?? '').trim() === 'codex_missing'
    const issue = detectCodexAuthIssueFromSessionError(payload)
    const mid = String(machineId ?? sessionRowsById.get(String(sessionId ?? '').trim())?.machineId ?? '').trim()
    const machine = mid ? machineRowsById.get(mid) : null
    if (issue) {
      const store = getSessionStore(String(sessionId ?? '').trim())
      if (store) {
        store.codexAuthIssue = {
          code: String(issue.code ?? '').trim() || null,
          message: String(issue.message ?? '').trim() || 'Codex needs sign-in on this runner.',
          machineId: mid || null,
          detectedAtMs: Date.now()
        }
      }
    }
    if (mid && (issue || missingCodex) && machineSupportsToolManagement(machine)) {
      loadMachineTools(mid, { force: true }).catch(() => {})
    }
    const machineName = machine ? machineDisplayName(machine) : 'this runner'
    if (issue) {
      const dedupeKey = `${mid || 'unknown'}:${issue.code}`
      if (machineCodexIssueToastSeen[dedupeKey]) return
      machineCodexIssueToastSeen[dedupeKey] = true

      showInlineToast({
        id: `codex-auth:${dedupeKey}`,
        level: 'warning',
        title: 'Codex Needs Sign-In',
        message: `${issue.message} Open Settings > Machines and sign in again for ${machineName}.`,
    ...
    if (!missingCodex) return

    setSessionCodexMissingIssue(sessionId, {
      code: 'codex_missing',
    ...
    const dedupeKey = `${mid || 'unknown'}:codex_missing`
    if (machineCodexIssueToastSeen[dedupeKey]) return
    machineCodexIssueToastSeen[dedupeKey] = true

    showInlineToast({
      id: `codex-missing:${dedupeKey}`,
      level: 'warning',
      title: 'Codex Missing',
      message: `Codex is not installed on ${machineName}. Open Settings > Machines and install managed Codex.`,
      sessionId: String(sessionId ?? '').trim() || null,
      ms: 12_000
    })
  }
...
watch(
  ...
  (enabled) => {
    if (enabled === true) notificationSoundPlayer.preload()
  },
  { immediate: true }
)
...
handleEnvelope = (env) => {
  ...
  const sessionId = String(env?.scope?.sessionId ?? env?.payload?.sessionId ?? '').trim()
  if (sessionId) syncSessionReadRequirement(sessionRowsById.get(sessionId) ?? null)
  if (env?.type === 'registry.snapshot') {
    refreshSelectedSessionQueuedPrompts().catch(() => {})
    return
  }
  if (shouldRefreshQueuedPromptsAfterEnvelope({
    env,
...
watch(selectedSessionId, async (sid, prevSid) => {
  ...
  if (String(prevSid ?? '').trim()) {
    persistWorkspacePaneState(prevSid)
  ...
  if (!sid) {
    restoreWorkspacePaneState(null)
...
watch(
  () => [selectedSessionId.value, workspacePaneOpen.value, workspacePaneTab.value],
  ([sid]) => {
    if (!String(sid ?? '').trim()) return
    persistWorkspacePaneState(sid)
  }
)
...
watch(defaultsOpen, (open) => {
  ...
  loadArchivedSessions().catch(() => {})
  maybeLoadSettingsMachineTools()
})

watch(
  () => ({
    open: defaultsOpen.value,
    tab: settingsTab.value,
    machines: machinesForSelect.value
      .map((machine) => `${machine?.machineId ?? ''}:${machine?.connected ? '1' : '0'}:${machineSupportsToolManagement(machine) ? '1' : '0'}`)
      .join('|')
  }),
  () => {
    maybeLoadSettingsMachineTools()
...
const visibleSessionWindow = computed(() => computeVirtualWindowVariable(sessionListEntries.value, {
  ...
  defaultItemHeight: SESSION_LIST_SESSION_SLOT_HEIGHT,
  overscanPx: 280,
  getItemHeight: (entry) => entry?.kind === 'group' ? COMPACT_SIDEBAR_GROUP_SLOT_HEIGHT : SESSION_LIST_SESSION_SLOT_HEIGHT
}))
...
const sessionChatShellStyle = computed(() => (
  isMobileLayout.value ? null : buildSessionChatShellStyle()
))
...
const selectedSessionMachineId = computed(() => {
  const mid = String(selectedSession.value?.machineId ?? '').trim()
  return mid || ''
})
const selectedSessionMachine = computed(() => {
  const mid = selectedSessionMachineId.value
  return mid ? (machineRowsById.get(mid) ?? null) : null
})
const selectedSessionCodexTool = computed(() => {
  const mid = selectedSessionMachineId.value
  if (!mid) return null
  return machineToolsByMachineId[mid]?.codex ?? null
})
const selectedSessionLegacyCodexIssue = computed(() => {
  const issue = selectedStore.value?.codexAuthIssue
  return (issue && typeof issue === 'object') ? issue : null
})
const selectedSessionCodexAuthTool = computed(() => (
  selectedSessionCodexTool.value?.auth
    ? selectedSessionCodexTool.value
    : buildLegacyCodexAuthTool(selectedSessionLegacyCodexIssue.value)
))
const selectedSessionCodexWebAuthSupported = computed(() => machineSupportsToolAuth(selectedSessionMachine.value))
const selectedSessionCodexSupportMessage = computed(() => (
  selectedSession.value ? machineCodexSupportMessage(selectedSessionMachine.value) : ''
))
const selectedSessionCodexInstallPromptVisible = computed(() => {
  if (!selectedSession.value || !selectedSessionMachine.value) return false
  if (!machineIsOnline(selectedSessionMachine.value)) return false
  if (!machineSupportsToolManagement(selectedSessionMachine.value)) return false
  if (selectedStore.value?.codexMissingPromptDismissed === true) return false

  const source = String(selectedSessionCodexTool.value?.source ?? '').trim()
  if (source) return source !== 'managed'
  return Boolean(selectedStore.value?.codexMissingIssue)
})
const selectedSessionCodexInstallActionLabel = computed(() => {
  const source = String(selectedSessionCodexTool.value?.source ?? '').trim()
  return source === 'external' ? 'Install managed Codex' : 'Install Codex'
})
const selectedSessionCodexInstallPromptMessage = computed(() => {
  const source = String(selectedSessionCodexTool.value?.source ?? '').trim()
  if (source === 'external') {
    return 'This runner only has an external Codex install. Install the managed copy Rootgrid uses for chat sessions and model listing.'
  }
  return String(selectedStore.value?.codexMissingIssue?.message ?? '').trim()
    || 'Managed Codex is missing on this runner. Install it to use chat sessions and model listing.'
})
const selectedSessionCodexAuthVisible = computed(() => {
  const tool = selectedSessionCodexAuthTool.value
  if (!selectedSession.value || !tool?.auth) return false
  if (!selectedSessionMachine.value) return false
  if (!machineIsOnline(selectedSessionMachine.value)) return false
  if (selectedSessionCodexWebAuthSupported.value && tool.installed && tool.auth?.flow?.active) return true
  const status = String(tool.auth?.status ?? '').trim()
  if (selectedSessionCodexWebAuthSupported.value) {
    return tool.installed && (status === 'reauth-required' || status === 'not-authenticated' || status === 'pending-browser-auth')
  }
  return Boolean(selectedSessionLegacyCodexIssue.value)
})

watch(
  () => ({
    machineId: selectedSessionMachineId.value,
    connected: Boolean(selectedSessionMachine.value?.connected),
    tools: machineSupportsToolManagement(selectedSessionMachine.value)
  }),
  () => {
    maybeLoadSelectedSessionMachineTools()
  },
  { immediate: true }
)
...
const composerMachineUpgradeWorking = computed(() => {
  ...
  return machineUpgradeWorking(machineId)
})
...
const settingsSidebarActive = computed(() => isSidebarSettingsActive(mainPaneMode.value))
function sidebarSessionEntryActive(sessionId) {
  return isSidebarSessionEntryActive({
    mainPaneMode: mainPaneMode.value,
    selectedSessionId: selectedSessionId.value,
    sessionId
  })
}
...
const composerRateLimitSource = computed(() => {
  if (selectedTokenUsage.value?.rateLimits) return selectedTokenUsage.value
  if (lastKnownRateLimits.value) return { rateLimits: lastKnownRateLimits.value }
  return null
})
const composerRateLimitSummary = computed(() => buildRateLimitSummary(composerRateLimitSource.value, nowMs.value))
const composerContextPopoverVisible = computed(() => (
  Boolean(composerContextUsage.value || composerRateLimitSummary.value) && (composerContextPopoverOpen.value || composerContextPopoverHover.value)
))
...
const workspacePaneHeaderAction = computed(() => {
  if (!mobileWorkspacePaneActive.value && !showWorkspacePane.value) return null
  ...
  if (mobileWorkspacePaneActive.value && workspacePaneTab.value === 'terminal' && workspaceTerminalSession.value?.terminalId) {
    return {
      kind: 'terminal-keys',
...
function runMobileWorkspaceHeaderAction() {
  const action = workspacePaneHeaderAction.value
...
function handleComposerMachineMouseEnter() {
  ...
  composerMachinePopoverClose.cancel()
  composerMachinePopoverHover.value = true
}

function handleComposerMachineMouseLeave() {
  composerMachinePopoverClose.schedule(() => {
    composerMachinePopoverHover.value = false
  })
}

function closeComposerMachinePopover() {
  composerMachinePopoverClose.cancel()
...
watch(
  () => ({
    ...
    machineId: selectedSessionMachineId.value,
    toolSource: String(selectedSessionCodexTool.value?.source ?? '').trim(),
    modelError: String(modelCatalogError.value ?? '').trim()
  }),
  ({ sessionId, machineId, toolSource, modelError }) => {
    const sid = String(sessionId ?? '').trim()
    if (!sid) return

    if (toolSource === 'managed') {
      clearSessionCodexMissingIssue(sid)
      return
    }

    if (toolSource === 'external') {
      setSessionCodexMissingIssue(sid, {
        code: 'codex_missing',
        message: 'This runner only has an external Codex install. Install the managed copy Rootgrid uses for chat sessions and model listing.',
    ...
    const missing = detectCodexMissingIssueText(modelError)
    if (missing) {
      setSessionCodexMissingIssue(sid, {
        ...missing,
        machineId: String(machineId ?? '').trim() || null,
...
const composerModelFallbackLabel = computed(() => resolveComposerModelFallbackLabel({
  currentModel: composerModel.value,
  defaultModel: defaultCodexModel.value,
  defaultLabel: defaultCodexModelLabel.value
}))
...
const {
  ...
  apiFetch,
  clearDraftForSession: clearComposerDraftForSession,
  defaults,
  defaultsError,
  openSettings,
  attachments,
...
function openSessionMenuClean(sessionId) {
  closeSessionMenu()
  openCleanModal(sessionId)
}
...
const {
  ...
  machineUpgradeWorkingIds,
  machineUpgradeError,
  appSettings,
  ideError,
  ideStarting,
  getMachineLabel: (machineId) => machineDisplayName(machineRowsById.get(machineId) ?? { machineId }),
...
const settingsUpgradeableMachines = computed(() => (
  machinesForSelect.value.filter((machine) => machineCanStartUpgrade(machine))
))

async function upgradeAllEligibleMachines() {
  const machineIds = settingsUpgradeableMachines.value.map((machine) => machine.machineId)
  if (!machineIds.length) return false
  return await upgradeMachines(machineIds)
}

function maybeLoadSettingsMachineTools({ force = false } = {}) {
  if (!defaultsOpen.value || settingsTab.value !== 'machines') return
  const machineIds = machinesForSelect.value
    .filter((machine) => Boolean(machine?.connected) && machineSupportsToolManagement(machine))
    .map((machine) => String(machine?.machineId ?? '').trim())
    .filter(Boolean)
  if (!machineIds.length) return
  loadMachineToolsForMachines(machineIds, { force }).catch(() => {})
}

function maybeLoadSelectedSessionMachineTools({ force = false } = {}) {
  const mid = selectedSessionMachineId.value
  if (!mid) return
  const machine = selectedSessionMachine.value
  if (!machine || !machine.connected || !machineSupportsToolManagement(machine)) return
  loadMachineTools(mid, { force }).catch(() => {})
}

function activeMachineToolAuthMachineIds() {
  const ids = []
  if (defaultsOpen.value && settingsTab.value === 'machines') {
    ids.push(...machinesForSelect.value
      .filter((machine) => Boolean(machine?.connected) && machineSupportsToolManagement(machine))
      .map((machine) => String(machine?.machineId ?? '').trim())
      .filter((machineId) => {
        const tools = machineToolsByMachineId[machineId]
        return Boolean(
          tools?.codex?.auth?.flow?.active
          || tools?.codeServer?.auth?.flow?.active
        )
      }))
  }

  const selectedMid = selectedSessionMachineId.value
  if (selectedMid) {
    const tools = machineToolsByMachineId[selectedMid]
    if (tools?.codex?.auth?.flow?.active || tools?.codeServer?.auth?.flow?.active) {
      ids.push(selectedMid)
    }
  }
...
function applyWorkspaceGitStatus(nextStatus) {
  if (!nextStatus || typeof nextStatus !== 'object') {
    workspaceGitStatus.value = nextStatus ?? null
    return
  }
  const current = workspaceGitStatus.value
  if (!current || typeof current !== 'object') {
    workspaceGitStatus.value = nextStatus
    return
  }
  for (const key of Object.keys(current)) {
    if (!(key in nextStatus)) delete current[key]
  }
  Object.assign(current, nextStatus)
}

function workspaceGitBusy() {
  return (
    workspaceGitLoading.value
    || workspaceGitActionWorking.value
    || workspaceGitBranchWorking.value
    || workspaceGitCommitWorking.value
    || workspaceGitPushWorking.value
  )
}

async function runWorkspaceGitMutation(pathname, body, { workingRef = workspaceGitActionWorking } = {}) {
  if (workspaceGitBusy()) return false
  ...
  try {
    ...
    if (
      data
      && typeof data === 'object'
      && (Array.isArray(data.entries) || typeof data.notRepo === 'boolean')
    ) {
      applyWorkspaceGitStatus(data)
    } else {
...
async function switchWorkspaceGitBranch(branchName) {
  ...
  return await runWorkspaceGitMutation('/api/git/branch/switch', {
    ...
    cwd: ctx.cwd,
    branch
  }, { workingRef: workspaceGitBranchWorking })
}
...
async function commitWorkspaceGitChanges() {
  const ctx = resolveWorkspaceContext()
  const message = String(workspaceGitCommitMessage.value ?? '').trim()
  if (!ctx || !message) return false
  const ok = await runWorkspaceGitMutation('/api/git/commit', {
    machineId: ctx.machineId,
    cwd: ctx.cwd,
    message
  }, { workingRef: workspaceGitCommitWorking })
  if (ok) workspaceGitCommitMessage.value = ''
  return ok
}

async function pushWorkspaceGitBranch() {
  const ctx = resolveWorkspaceContext()
  if (!ctx) return false
  return await runWorkspaceGitMutation('/api/git/push', {
    machineId: ctx.machineId,
    cwd: ctx.cwd
  }, { workingRef: workspaceGitPushWorking })
}

async function openMarkdownFile(target) {
  const path = String(target?.path ?? target ?? '').trim()
  if (!path) return false
  activateWorkspacePane('files')
  const dir = parentDirectoryPath(path)
  if (dir) await loadWorkspaceFiles(dir).catch(() => {})
  return await loadWorkspaceFile(path)
}
...
onMounted(async () => {
  ...
  const onKeyDown = (ev) => {
    if (ev.key === 'Escape') {
      ...
      else if (cleanOpen.value) closeCleanModal()
  ...
  const onSessionMenuOutside = (ev) => {
    ...
    if (!inMachinePopover) closeComposerMachinePopover()
  ...
  const onReadInteraction = () => {
    sessionReadGate.noteInteraction()
    const sid = String(selectedSessionId.value ?? '').trim()
    if (sid && stickToBottom.value) scheduleMarkRead(sid)
  }
  readInteractionHandler = onReadInteraction
  try { document.addEventListener('click', onReadInteraction) } catch {}
  ...
  machineToolAuthPollTimer = setInterval(() => {
    if (!authed.value) return
    const machineIds = activeMachineToolAuthMachineIds()
    if (!machineIds.length) return
    loadMachineToolsForMachines(machineIds, { force: true }).catch(() => {})
  }, 2_000)
...
onBeforeUnmount(() => {
  ...
  if (machineToolAuthPollTimer) {
    try { clearInterval(machineToolAuthPollTimer) } catch {}
    machineToolAuthPollTimer = null
  }
  if (keydownHandler) {
  ...
  if (readInteractionHandler) {
    try { document.removeEventListener('click', readInteractionHandler) } catch {}
    readInteractionHandler = null
  }
  composerMachinePopoverClose.dispose()
})
...
watch(
  ...
  ([machineId, cwd], [prevMachineId, prevCwd]) => {
    ...
    workspaceGitError.value = ''
    workspaceGitBranchDraft.value = ''
    workspaceGitCommitMessage.value = ''
    const terminalSession = workspaceTerminalSession.value
    const paneOpen = Boolean(workspacePaneOpen.value)
    const paneTab = normalizeWorkspacePaneTab(workspacePaneTab.value)
    if (paneOpen && paneTab === 'git' && String(cwd ?? '').trim()) {
      refreshWorkspaceGit().catch(() => {})
    } else if (paneOpen && paneTab === 'files' && String(cwd ?? '').trim()) {
      loadWorkspaceFiles().catch(() => {})
    } else if (paneOpen && paneTab === 'code' && String(cwd ?? '').trim()) {
      openWorkspace().catch(() => {})
    }
    ...
    closeWorkspaceTerminalSession().then(() => {
      if (paneOpen && paneTab === 'terminal' && String(cwd ?? '').trim()) {
  ...
  <div class="rg-app-shell bg-[#f7f7f4] text-slate-900" :style="appShellStyle">
    ...
    <div v-else class="h-full flex flex-col bg-white">
                ...
                <template v-for="entry in visibleSessionWindow.items" :key="entry.key">
                  ...
                  >
                    <button
                      ...
                      :class="sidebarSessionEntryActive(entry.session.sessionId)
            ...
            <div class="relative z-20 shrink-0 p-2">
              <div class="flex flex-col items-center space-y-1.5">
                <div class="flex justify-center" :style="compactSidebarFooterPrimarySlotStyle">
                  <button
                    class="inline-flex h-8 w-8 items-center justify-center rounded-xl transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-slate-400/30"
                    :class="settingsSidebarActive
                      ? 'bg-white text-slate-700 shadow-sm ring-1 ring-black/[0.08]'
          ...
          >
              ...
              <div v-else>
                ...
                <div v-else>
                  ...
                  <template v-for="entry in visibleSessionWindow.items" :key="entry.key">
                    ...
                    <div v-else class="flex items-center" :style="expandedSidebarSessionSlotStyle">
                      <button
                        ...
                        :class="[
                          ...
                          sessionListAccentClass(entry.session, {
                            selected: sidebarSessionEntryActive(entry.session.sessionId)
                          })
                        ]"
                        :title="sessionTooltip(entry.session)"
                        @click="openSession(entry.session.sessionId)"
                              ...
                              <div class="flex shrink-0 items-center gap-1">
                                ...
                                <div class="relative shrink-0" data-session-menu-root="true">
                                  ...
                                  <div
                                    ...
                                    class="absolute right-0 top-7 z-20 w-40 rounded-xl border border-black/[0.08] bg-white p-1 shadow-lg"
                                  >
                                    <button
                                    ...
                                    <button
                                      class="w-full rounded-lg px-3 py-2 text-left text-sm text-slate-700 transition-colors hover:bg-slate-100"
                                      @click.stop="openSessionMenuClean(entry.session.sessionId)"
                                    >
                                      Clean session
                                    </button>
                                    <button
            ...
            <div class="relative z-20 shrink-0 p-2">
              <div class="space-y-1.5">
                <button
                  class="min-w-0 inline-flex w-full items-center gap-2.5 rounded-lg px-2.5 py-1.5 text-left text-[13px] transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-slate-400/30"
                  :class="settingsSidebarActive
                ...
                <div v-if="!isMobileLayout" class="relative w-full" data-sidebar-mode-root="true">
                  <button
                    type="button"
                    class="inline-flex h-8 w-full items-center gap-1.5 rounded-lg px-2.5 text-left text-[12px] text-slate-500 transition-colors hover:bg-black/[0.04] hover:text-slate-700 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-slate-400/30"
                    title="Sidebar control"
                    @click.stop="toggleDesktopSidebarModeMenu"
                  >
        ...
        >
          ...
          <div v-if="mainPaneMode === 'chat'" class="relative flex-1 min-h-0">
            ...
            >
              ...
              <div v-else :key="selectedSessionId ? `chat-${selectedSessionId}` : 'chat-empty'">
                          ...
                          <template v-for="it in m.timeline" :key="it.id">
                            ...
                            >
                              ...
                              <div v-if="String(it.section?.body ?? '').trim()" class="mt-2 border-l border-slate-200 pl-4">
                                <MarkdownView :source="it.section.body" @open-file="openMarkdownFile" />
                              </div>
                            </details>
                            <details
                            ...
                            <div
                              v-else-if="it.kind === 'stderr'"
                              class="overflow-hidden rounded-xl border border-black/[0.06] bg-[#f3f3f0]"
                            >
                              <div class="border-b border-black/[0.05] px-3 py-2 text-[10px] uppercase tracking-wider text-slate-500">
                                stderr
                              </div>
                              <pre class="rg-chat-local-x-scroll m-0 max-h-64 overflow-auto whitespace-pre-wrap px-3 py-2 text-xs font-mono text-slate-800">{{ it.text }}</pre>
                            </div>
                          </template>
                  ...
                  >
                    <div v-if="m.role === 'assistant'" class="min-w-0 max-w-full overflow-hidden">
                      <div v-if="String(m.text ?? '').trim()" class="min-w-0 max-w-full overflow-hidden">
                        <MarkdownView :source="m.text" @open-file="openMarkdownFile" />
                      </div>
                    </div>
          ...
          >
            <div class="mx-auto w-full" :style="sessionChatShellStyle">
              ...
              <div class="overflow-hidden rounded-[24px] border border-black/[0.06] bg-white shadow-[0_1px_2px_rgba(0,0,0,0.05)] transition-colors focus-within:border-black/[0.12] focus-within:ring-2 focus-within:ring-slate-400/20">
                          ...
                          <div class="flex items-center justify-between gap-3">
                            <div class="min-w-0 flex-1">
                              <textarea
                                ...
                                rows="2"
                                class="max-h-[4.5rem] w-full resize-none overflow-y-auto rounded-xl border border-black/[0.06] bg-white px-3 py-2 text-[13px] leading-6 text-slate-800 outline-none placeholder:text-slate-400 focus:border-black/[0.12]"
                              ...
                              <div v-else class="min-w-0">
                                <div class="max-h-[4.5rem] overflow-y-auto whitespace-pre-wrap break-words pr-1 text-[13px] leading-6 text-slate-900">
                ...
                <div
                  v-if="selectedSessionCodexInstallPromptVisible"
                  class="border-b border-amber-200 bg-amber-50/80 px-4 py-3"
                >
                  <div class="flex items-start gap-3">
                    <div class="min-w-0 flex-1">
                      <div class="text-[11px] uppercase tracking-wider text-amber-700">Managed Codex Required</div>
                      <div class="mt-1 text-sm text-amber-950">
                        {{ selectedSessionCodexInstallPromptMessage }}
                      </div>
                      <div v-if="machineToolErrorText(selectedSessionMachineId, 'codex')" class="mt-2 text-xs text-red-700">
                        {{ machineToolErrorText(selectedSessionMachineId, 'codex') }}
                      </div>
                      <div v-else-if="modelCatalogError" class="mt-2 text-xs text-amber-700">
                        {{ modelCatalogError }}
                      </div>
                      <div class="mt-3">
                        <button
                          type="button"
                          class="inline-flex items-center gap-2 rounded-full bg-slate-900 px-3 py-1.5 text-xs font-medium text-white transition-colors hover:bg-slate-800 disabled:cursor-not-allowed disabled:opacity-40"
                          :disabled="machineToolWorking(selectedSessionMachineId, 'codex')"
                          @click="installSelectedSessionCodex"
                        >
                          <Loader2 v-if="machineToolWorking(selectedSessionMachineId, 'codex')" class="h-3.5 w-3.5 animate-spin" />
                          {{ selectedSessionCodexInstallActionLabel }}
                        </button>
                      </div>
                    </div>
                    <button
                      type="button"
                      class="inline-flex h-7 w-7 shrink-0 items-center justify-center rounded-full bg-white/80 text-amber-700 ring-1 ring-amber-200 transition-colors hover:bg-white"
                      title="Dismiss"
                      @click="dismissSelectedSessionCodexInstallPrompt"
                    >
                      <X class="h-4 w-4" />
                    </button>
                  </div>
                ...
                <div
                  v-if="selectedSessionCodexAuthVisible"
                  class="border-b border-black/[0.06] bg-[#f5f5f2] px-4 py-3"
                >
                  <CodexAuthPanel
                    :tool="selectedSessionCodexAuthTool"
                    :web-auth-supported="selectedSessionCodexWebAuthSupported"
                    :support-message="selectedSessionCodexSupportMessage"
                    :busy="machineToolAuthBusy(selectedSessionMachineId, 'codex')"
                    :auth-error-text="machineToolAuthErrorText(selectedSessionMachineId, 'codex')"
                    :auth-status-text="machineToolAuthStatusText(selectedSessionMachineId, 'codex')"
                    @refresh="refreshMachineToolAuth(selectedSessionMachineId, 'codex')"
                    @start-device-auth="startMachineToolDeviceAuth(selectedSessionMachineId, 'codex')"
                    @cancel-device-auth="cancelMachineToolDeviceAuth(selectedSessionMachineId, 'codex')"
                    @logout="logoutMachineTool(selectedSessionMachineId, 'codex')"
                  />
                </div>
                ...
                <div class="mt-2 flex items-center justify-between gap-2">
                  <div class="flex min-w-0 items-center gap-0.5 overflow-x-auto pb-1 sm:gap-1">
                    ...
                    <ComposerPillSelect
                      ...
                      title="Choose model"
                      :fallback-label="composerModelFallbackLabel"
                    ...
                    <button
                      type="button"
                      class="inline-flex h-7 w-7 shrink-0 items-center justify-center rounded-full transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-slate-400/40 sm:h-8 sm:w-8"
                      :class="composerFastMode
                        ? 'bg-transparent text-slate-700 hover:bg-black/[0.04]'
                        : 'bg-transparent text-slate-700 hover:bg-black/[0.04]'"
                      :title="composerFastMode
                        ? 'Disable Codex fast mode.'
                        : 'Enable Codex fast mode.'"
                      @click="composerFastMode = !composerFastMode"
                    >
                      <Zap
                        class="h-3 w-3 transition-all sm:h-3.5 sm:w-3.5"
                        :class="composerFastMode
                          ? 'scale-110 text-blue-700 drop-shadow-[0_0_0.2rem_rgba(29,78,216,0.28)]'
                          : 'text-slate-400'"
                      />
                    </button>
              ...
              <div class="mt-2 flex flex-wrap items-center justify-between gap-2 px-1 text-[11px] text-slate-400">
                <div class="flex min-w-0 flex-wrap items-center gap-1 sm:gap-1.5">
                  <div
                    ...
                    @mouseleave="handleComposerMachineMouseLeave"
                  >
                    <button
                      type="button"
                ...
                <div class="flex items-center self-center gap-2">
                  <div
                    v-if="composerContextUsage || composerRateLimitSummary"
                  ...
                  >
                    <button
                      ...
                      type="button"
                      :title="composerContextUsage?.usageLabel || 'Rate limits remaining'"
                    ...
                    <div
                      ...
                      class="absolute bottom-8 right-0 z-20 w-56 rounded-2xl border border-black/[0.06] bg-white px-3 py-2 text-center shadow-lg shadow-black/10"
                    >
                      <template v-if="composerContextUsage">
                      ...
                      <div
                        v-if="composerRateLimitSummary"
                        class="space-y-1.5"
                        :class="composerContextUsage ? 'mt-2 border-t border-black/[0.06] pt-2 text-left' : 'text-left'"
                      >
                        <div class="flex items-center justify-between gap-2 text-[11px] text-slate-500">
                          <span>Rate limits remaining</span>
                          <span v-if="composerRateLimitSummary.planType" class="uppercase tracking-wide text-slate-400">
                            {{ composerRateLimitSummary.planType }}
                          </span>
                        </div>
                        <div
                          v-for="item in composerRateLimitSummary.items"
                          :key="`${item.label}-${item.resetAtMs ?? 'none'}`"
                          class="grid grid-cols-[1fr_auto_auto] items-center gap-3 text-xs"
                        >
                          <span class="truncate text-slate-700">{{ item.label }}</span>
                          <span class="font-medium text-slate-900">{{ item.remainingLabel }}</span>
                          <span class="text-[11px] text-slate-500">{{ item.resetLabel }}</span>
                        </div>
                      </div>
                    </div>
                  </div>
          ...
          <div
            ...
            :class="mainPaneMode === 'new-thread'
              ? (isMobileLayout ? 'overflow-y-auto' : 'overflow-hidden')
              : 'overflow-auto'"
          >
            <div
              class="mx-auto flex min-h-0 w-full max-w-3xl flex-col"
              :class="mainPaneMode === 'new-thread' && isMobileLayout ? 'min-h-full' : 'h-full'"
            >
              ...
              <div v-else class="space-y-8">
                <section class="space-y-3">
                  ...
                  <div class="flex items-center justify-between gap-3 rounded-xl border border-slate-200 bg-slate-50 p-3">
                    <div class="min-w-0">
                      ...
                      <div class="mt-1 text-xs text-slate-700">Move the thread list to the right</div>
                      <div class="mt-1 text-xs text-slate-500">Keeps the same session list layout and only changes which side it sits on for this device.</div>
                    </div>
                    <button
                      type="button"
                      role="switch"
                  ...
                  <div>
                    ...
                    <div class="mt-3">
                      <div class="flex items-center justify-between gap-3 rounded-xl border border-slate-200 bg-slate-50 p-3">
                        <div class="min-w-0">
                          <div class="text-[11px] uppercase tracking-wider text-slate-500">Sound notifications</div>
                          <div class="mt-1 text-xs text-slate-700">Play the bundled alert sound for incoming approvals and completion toasts.</div>
                        </div>
                        <button
                          type="button"
                          role="switch"
                          :aria-checked="soundNotificationsDraft ? 'true' : 'false'"
                          class="relative inline-flex h-7 w-12 shrink-0 rounded-full transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-indigo-500/30"
                          :class="soundNotificationsDraft ? 'bg-slate-900' : 'bg-slate-300'"
                          @click="soundNotificationsDraft = !soundNotificationsDraft"
                        >
                          <span
                            class="absolute left-0 top-1 h-5 w-5 rounded-full bg-white shadow-sm transition-transform"
                            :class="soundNotificationsDraft ? 'translate-x-6' : 'translate-x-1'"
                          />
                        </button>
                      </div>
                    </div>
                  ...
                  <div class="rounded-xl border border-slate-200 bg-slate-50 p-3">
                      ...
                      <div class="min-w-0">
                        ...
                        <div class="mt-2 flex flex-wrap items-center gap-2 text-[11px]">
                          <span
                            ...
                            :class="hostSelfUpdateUpdateAvailable
                              ? 'bg-amber-50 text-amber-700 ring-1 ring-amber-200'
                              : hostSelfUpdateUpToDate
                                ? 'bg-emerald-50 text-emerald-700 ring-1 ring-emerald-200'
                                : hostSelfUpdateLatestCheckError
                                  ? 'bg-amber-50 text-amber-700 ring-1 ring-amber-200'
                                  : 'bg-slate-100 text-slate-600 ring-1 ring-slate-200'"
                          >
                            {{ hostSelfUpdateUpdateAvailable
                              ? 'Update available'
                              : hostSelfUpdateUpToDate
                                ? 'Up to date'
                                : hostSelfUpdateLatestCheckError
                          ...
                          <span v-if="hostSelfUpdateLatestInfo?.releaseName" class="truncate text-slate-500">
                            {{ hostSelfUpdateLatestInfo.releaseName }}
                          </span>
                        </div>
                      </div>
                      <button
                        ...
                        :title="hostSelfUpdateEnabled
                          ? (hostSelfUpdateUpToDate
                              ? 'This host already matches the latest release. Click to reinstall it anyway.'
                              : 'Download the configured GitHub release bundle and restart the host.')
                          : hostSelfUpdateDisabledReason === 'missing-repo'
                            ? 'Set host.selfUpdate.repo or ROOTGRID_GITHUB_REPO to enable web-triggered host updates.'
                            : hostSelfUpdateDisabledReason === 'disabled'
                              ? 'Host self-update is disabled.'
                              : 'Host self-update is unavailable.'"
                    ...
                    <div class="mt-3 grid grid-cols-1 gap-2 text-xs text-slate-500 sm:grid-cols-2">
                      <div class="rounded-xl border border-slate-200 bg-white px-3 py-2">
                        Installed version:
                        <span class="ml-1 break-all font-mono text-slate-700">{{ hostSelfUpdateReleaseVersion(hostSelfUpdateInstalledInfo) }}</span>
                      </div>
                      <div class="rounded-xl border border-slate-200 bg-white px-3 py-2" :title="hostSelfUpdateReleaseHash(hostSelfUpdateInstalledInfo) ?? ''">
                        Installed hash:
                        <span class="ml-1 break-all font-mono text-slate-700">{{ hostSelfUpdateReleaseHashShort(hostSelfUpdateInstalledInfo) }}</span>
                      </div>
                      <div class="rounded-xl border border-slate-200 bg-white px-3 py-2">
                        Latest version:
                        <span class="ml-1 break-all font-mono text-slate-700">{{ hostSelfUpdateReleaseVersion(hostSelfUpdateLatestInfo, '—') }}</span>
                      </div>
                      <div class="rounded-xl border border-slate-200 bg-white px-3 py-2" :title="hostSelfUpdateReleaseHash(hostSelfUpdateLatestInfo) ?? ''">
                        Latest hash:
                        <span class="ml-1 break-all font-mono text-slate-700">{{ hostSelfUpdateReleaseHashShort(hostSelfUpdateLatestInfo) }}</span>
                      </div>
                      <div class="rounded-xl border border-slate-200 bg-white px-3 py-2">
                        Installed release:
                        <span class="ml-1 break-all font-mono text-slate-700">{{ hostSelfUpdateInstalledInfo?.releaseId ?? '—' }}</span>
                      </div>
                      <div class="rounded-xl border border-slate-200 bg-white px-3 py-2">
                        Latest checked:
                        <span class="ml-1 font-mono text-slate-700">{{ hostSelfUpdateLatestInfo?.checkedAtMs ? `${formatAgeShort(hostSelfUpdateLatestInfo.checkedAtMs)} ago` : '—' }}</span>
                      </div>
                      <div class="rounded-xl border border-slate-200 bg-white px-3 py-2">
                        Repository:
                    ...
                    <div v-else class="mt-2 text-xs text-slate-500">
                      Restart mode: Rootgrid re-execs itself into the new managed release after the update finishes.
                    </div>
                    ...
                    <div v-else-if="hostSelfUpdateLatestCheckError" class="mt-2 text-xs text-amber-700">{{ hostSelfUpdateLatestCheckError }}</div>
                    <div v-else-if="hostSelfUpdateDisabledReason === 'missing-repo'" class="mt-2 text-xs text-amber-700">
                      Configure <span class="font-mono">host.selfUpdate.repo</span> or set <span class="font-mono">ROOTGRID_GITHUB_REPO</span> to enable this button.
                    </div>
                    <div v-else-if="hostSelfUpdateDisabledReason === 'disabled'" class="mt-2 text-xs text-amber-700">
                      Host self-update is disabled. Enable it with <span class="font-mono">host.selfUpdate.enabled=true</span> or <span class="font-mono">ROOTGRID_ENABLE_HOST_SELF_UPDATE=1</span>.
                    </div>
                  </div>
                ...
                <section class="space-y-3">
                  <div class="flex flex-wrap items-start justify-between gap-3">
                    ...
                    <button
                      type="button"
                      class="inline-flex items-center gap-2 rounded-md bg-indigo-50 px-3 py-1.5 text-xs text-indigo-700 transition-colors hover:bg-indigo-100 disabled:cursor-not-allowed disabled:opacity-40 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-indigo-500/30"
                      :disabled="!settingsUpgradeableMachines.length"
                      :title="settingsUpgradeableMachines.length ? `Upgrade ${settingsUpgradeableMachines.length} eligible ${pluralize(settingsUpgradeableMachines.length, 'runner')}` : 'No eligible runners to upgrade right now.'"
                      @click="upgradeAllEligibleMachines"
                    >
                  ...
                  <div v-else class="space-y-2">
                    ...
                    >
                      <div class="flex items-center justify-between gap-3">
                        ...
                        <div class="shrink-0 flex items-center gap-2">
                          <button
                            v-if="machineUpgradeAvailable(m)"
                            ...
                            :disabled="Boolean(machineUpgradeLockReason(m)) || machineUpgradeWorking(m.machineId) || machineUpgradeBusy(m)"
                            :title="machineUpgradeButtonTitle(m)"
                          >
                            <Loader2 v-if="machineUpgradeWorking(m.machineId)" class="h-3.5 w-3.5 animate-spin" />
                      ...
                      <div class="mt-3 rounded-xl border border-slate-200 bg-slate-50 p-3">
                        <div class="flex items-start justify-between gap-3">
                          <div class="min-w-0">
                            <div class="text-[11px] uppercase tracking-wider text-slate-500">Runner tools</div>
                            <div class="mt-1 text-xs text-slate-500">
                              Rootgrid keeps managed Codex and code-server installs on each runner and prefers those over external binaries.
                            </div>
                          </div>
                          <button
                            v-if="m.connected && machineSupportsToolManagement(m)"
                            ...
                            :disabled="machineToolsLoading(m.machineId)"
                            @click="loadMachineTools(m.machineId, { force: true })"
                            title="Refresh Codex and code-server versions from this runner."
                          >
                            <RotateCw class="h-3.5 w-3.5" :class="machineToolsLoading(m.machineId) ? 'animate-spin' : ''" />
                            Refresh
                          </button>
                        </div>

                        <div v-if="!m.connected" class="mt-2 text-xs text-slate-500">
                          Connect the runner to inspect Codex and code-server.
                        </div>
                        <div v-else-if="!machineSupportsToolManagement(m)" class="mt-2 text-xs text-amber-700">
                          This runner does not support web-managed tool installs yet. Update the runner first.
                        </div>
                        <div v-else-if="machineToolsError(m.machineId)" class="mt-2 text-xs text-red-600">
                          {{ machineToolsError(m.machineId) }}
                        </div>
                        <div v-else-if="machineToolsLoading(m.machineId) && !machineToolsFor(m.machineId).length" class="mt-2 text-xs text-slate-500">
                          Loading tool versions…
                        </div>
                        <div v-else class="mt-3 grid grid-cols-1 gap-2 sm:grid-cols-2">
                          <div
                            v-for="tool in machineToolsFor(m.machineId)"
                            :key="`${m.machineId}:${tool.id}`"
                            class="rounded-xl border border-slate-200 bg-white p-3"
                          >
                            <div class="flex items-start justify-between gap-3">
                              <div class="min-w-0">
                                <div class="text-sm font-medium text-slate-900">{{ tool.label }}</div>
                                <div class="mt-1 text-xs text-slate-500">
                                  Version:
                                  <span class="ml-1 font-mono text-slate-700">{{ tool.version ?? 'not installed' }}</span>
                                </div>
                                <div class="mt-1 text-xs text-slate-500">
                                  Source:
                                  <span class="ml-1 text-slate-700">{{ tool.source === 'managed' ? 'managed' : (tool.source === 'external' ? 'external' : 'missing') }}</span>
                                </div>
                                <div v-if="tool.path" class="mt-2 break-all text-[11px] text-slate-500">
                                  Path:
                                  <span class="ml-1 font-mono text-slate-700">{{ tool.path }}</span>
                                </div>
                                <div class="mt-2 break-all text-[11px] text-slate-500">
                                  Command:
                                  <span class="ml-1 font-mono text-slate-700">{{ tool.upgradeCommand }}</span>
                                </div>
                              </div>
                              <button
                                class="inline-flex shrink-0 items-center gap-2 rounded-md bg-slate-900 px-3 py-2 text-xs font-medium text-white transition-colors hover:bg-slate-800 disabled:cursor-not-allowed disabled:opacity-40"
                                :disabled="machineToolWorking(m.machineId, tool.id)"
                                :title="`${machineToolActionLabel(tool)} ${tool.label} on ${machineDisplayName(m)}.`"
                                @click="upgradeMachineTool(m.machineId, tool.id)"
                              >
                                <Loader2 v-if="machineToolWorking(m.machineId, tool.id)" class="h-3.5 w-3.5 animate-spin" />
                                {{ machineToolActionLabel(tool) }}
                              </button>
                            </div>
                            <div class="mt-2 break-all text-[11px] text-slate-500">
                              Docs:
                              <a class="ml-1 text-indigo-700 hover:text-indigo-600 hover:underline" :href="tool.docsUrl" target="_blank" rel="noreferrer">
                                {{ tool.docsUrl }}
                              </a>
                            </div>
                            <div v-if="machineToolErrorText(m.machineId, tool.id)" class="mt-2 text-xs text-red-600">
                              {{ machineToolErrorText(m.machineId, tool.id) }}
                            </div>
                            <div v-else-if="machineToolStatusText(m.machineId, tool.id)" class="mt-2 text-xs text-emerald-700">
                              {{ machineToolStatusText(m.machineId, tool.id) }}
                            </div>
                          </div>
                        </div>
                        <CodexAuthPanel
                          v-if="machineCodexTool(m.machineId)?.auth"
                          class="mt-3"
                          :tool="machineCodexTool(m.machineId)"
                          :web-auth-supported="machineSupportsToolAuth(m)"
                          :support-message="machineCodexSupportMessage(m)"
                          :busy="machineToolAuthBusy(m.machineId, 'codex')"
                          :auth-error-text="machineToolAuthErrorText(m.machineId, 'codex')"
                          :auth-status-text="machineToolAuthStatusText(m.machineId, 'codex')"
                          @refresh="refreshMachineToolAuth(m.machineId, 'codex')"
                          @start-device-auth="startMachineToolDeviceAuth(m.machineId, 'codex')"
                          @cancel-device-auth="cancelMachineToolDeviceAuth(m.machineId, 'codex')"
                          @logout="logoutMachineTool(m.machineId, 'codex')"
                        />
                      </div>
                    </div>
                  </div>
        ...
        >
          <div
            :class="mobileWorkspaceLandscapeHeader
              ? 'flex w-[30px] shrink-0 flex-col items-center gap-1 border-r border-black/[0.05] px-[3px] py-1'
              : 'flex h-[30px] items-center justify-between gap-1.5 px-2'"
          >
            <template v-if="mobileWorkspaceLandscapeHeader">
              <button
                class="inline-flex h-5 w-5 shrink-0 items-center justify-center rounded-full text-slate-500 transition-colors hover:bg-black/[0.04] hover:text-slate-800 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-slate-400/30"
                title="Back to chat"
              ...
              <div class="flex min-h-0 flex-1 items-center justify-center overflow-hidden">
                <div
                  class="max-h-full overflow-hidden text-[9px] font-medium leading-none text-slate-700"
              ...
              <button
                v-if="workspacePaneHeaderAction"
                class="inline-flex h-5 w-5 shrink-0 items-center justify-center rounded-full border border-black/[0.06] bg-white text-slate-600 transition-colors hover:bg-black/[0.03] hover:text-slate-900 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-slate-400/30 disabled:cursor-not-allowed disabled:opacity-45"
                type="button"
                :title="workspacePaneHeaderAction.title"
                :disabled="workspacePaneHeaderAction.disabled"
                @click="runMobileWorkspaceHeaderAction"
              >
                <Loader2
                  v-if="workspacePaneHeaderAction.kind === 'refresh-files' && workspaceFilesLoading"
                  class="h-3 w-3 animate-spin"
                />
                <Loader2
                  v-else-if="workspacePaneHeaderAction.kind === 'refresh-git' && workspaceGitLoading"
                  class="h-3 w-3 animate-spin"
                />
                <Keyboard
                  v-else-if="workspacePaneHeaderAction.kind === 'terminal-keys'"
                  class="h-3 w-3"
                />
            ...
            <template v-else>
              <div class="flex min-w-0 items-center gap-2">
                ...
                <div class="min-w-0">
                  <div class="truncate text-[10px] font-semibold leading-none text-slate-800">{{ currentWorkspaceHostLabel }}</div>
                  <div class="truncate text-[8px] leading-none text-slate-500" :title="currentWorkspaceContext.cwd">{{ currentWorkspaceContext.cwd }}</div>
                </div>
              </div>
          ...
          <div class="min-h-0 flex-1 min-w-0">
            ...
            <WorkspaceGitPane
              ...
              :commit-working="workspaceGitCommitWorking"
              :push-working="workspaceGitPushWorking"
              :branch-draft="workspaceGitBranchDraft"
              :commit-message="workspaceGitCommitMessage"
              @refresh="refreshWorkspaceGit"
              @open-file="openGitFile"
              @stage="stageWorkspaceGitPaths"
              @unstage="unstageWorkspaceGitPaths"
              @commit="commitWorkspaceGitChanges"
              @push="pushWorkspaceGitBranch"
              ...
              @update:commit-message="workspaceGitCommitMessage = $event"
            />
          </div>
        </section>
      </main>
      ...
      <div
        ...
        class="group relative shrink-0 w-4 cursor-col-resize touch-none"
        title="Drag to resize the chat and workspace panes"
        @pointerdown.prevent="beginWorkspacePaneDrag"
      >
        <div
          class="absolute inset-y-3 left-1/2 w-px -translate-x-1/2 rounded-full transition-colors"
          :class="workspaceSplitDragging ? 'bg-slate-500/90' : 'bg-black/[0.12] group-hover:bg-slate-400/90'"
        />
      </div>
      ...
      >
        <header class="shrink-0 border-b border-black/[0.05] bg-white px-2">
          <div class="flex h-[30px] min-w-0 items-center justify-between gap-1.5">
            <div class="min-w-0 flex-1">
              <div class="truncate text-[10px] font-semibold leading-none text-slate-800">{{ currentWorkspaceHostLabel }}</div>
              <div class="truncate text-[8px] leading-none text-slate-500" :title="currentWorkspaceContext.cwd">{{ currentWorkspaceContext.cwd }}</div>
            </div>
            ...
            <div class="flex shrink-0 items-center gap-1.5">
              ...
              <button
                class="inline-flex h-5 w-5 items-center justify-center rounded-full border border-black/[0.06] bg-white text-slate-500 transition-colors hover:bg-black/[0.03] hover:text-slate-800 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-slate-400/30"
      ...
      <!-- Delete machine modal -->
	      ...
	      <!-- Clean session modal -->
	      <transition name="rg-fade">
	        <div v-if="cleanOpen" class="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-6">
	          <div class="w-full max-w-xl rounded-2xl border border-slate-200 bg-white p-5 shadow-2xl">
	            <div class="text-sm font-semibold text-slate-900">Clean session?</div>
	            <div class="mt-1 text-xs text-slate-600">
	              This creates a fresh session in the same folder and opens it. Then it deletes this session. Choose Archive if you want to keep the old session instead.
	            </div>

	            <div class="mt-4 rounded-xl border border-slate-200 bg-slate-50 p-3">
	              <div class="truncate text-sm font-medium text-slate-900">
	                {{ cleanSessionRow ? sessionListTitle(cleanSessionRow) : 'Selected thread' }}
	              </div>
	              <div class="mt-1 truncate text-xs text-slate-600" :title="cleanSessionRow?.cwd ?? ''">
	                {{ cleanSessionRow?.cwd ?? 'Unknown folder' }}
	              </div>
	            </div>

	            <div v-if="cleanError" class="mt-3 text-sm text-red-600">{{ cleanError }}</div>

	            <div class="mt-5 flex items-center justify-end gap-2">
	              <button
	                class="rounded-md bg-slate-100 px-3 py-2 text-sm text-slate-800 transition-colors hover:bg-slate-200 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-indigo-500/30"
	                @click="closeCleanModal"
	                :disabled="cleanWorking"
	              >
	                No
	              </button>
	              <button
	                class="rounded-md bg-amber-100 px-3 py-2 text-sm font-medium text-amber-800 transition-colors hover:bg-amber-200 disabled:cursor-not-allowed disabled:opacity-40 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-amber-500/30"
	                @click="confirmCleanSessionArchive"
	                :disabled="cleanWorking"
	              >
	                Archive
	              </button>
	              <button
	                class="inline-flex items-center gap-2 rounded-md bg-red-600 px-3 py-2 text-sm font-medium text-white transition-colors hover:bg-red-500 disabled:opacity-40 disabled:cursor-not-allowed focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-red-500/30"
	                @click="confirmCleanSessionDelete"
	                :disabled="cleanWorking"
	              >
	                <Loader2 v-if="cleanWorking" class="h-4 w-4 animate-spin" />
	                Yes
	              </button>
	            </div>
	          </div>
